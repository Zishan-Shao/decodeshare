...
HIDDEN_DIM = 4096  # Hidden dimension (4096 for Llama-2-7b, adjust for other models)
...
def find_fully_shared_basis_improved(task_contributions, all_tasks, cross_subspace_dim,
                                 min_tasks_shared=None, relative_threshold=0.001,
    ...
    for comp_idx in range(num_components_to_check):
        ...
        for task_name in all_tasks:
            if (task_name in task_contributions and
                'raw_variances' in task_contributions[task_name] and
                len(task_contributions[task_name]['raw_variances']) > comp_idx):
        ...
        if significant_tasks:
            component_task_significance[comp_idx] = {
                'tasks': significant_tasks,
                'count': len(significant_tasks)
            }
    ...
    if shared_indices:
        ...
        for idx in shared_indices:
            ...
            for task_name in all_tasks:
                if (task_name in task_contributions and
                    'raw_variances' in task_contributions[task_name] and
                    len(task_contributions[task_name]['raw_variances']) > idx):
...
def perform_hypothesis_test(baseline_losses, treatment_losses, test_name="test", alpha=0.05):
    ...
    if std1 == 0 and std2 == 0:
        ...
        if abs(t_stat) > 10:  # For very large t-statistics, p-value is essentially 0
            ...
            # Approximate using the fact that t-distribution approaches normal for large |t|
            # but is more conservative (higher p-values) for small df
            if abs(t_stat) < 2:
                # For small t, use the approximation
                p_value = 2 * (1 - 0.5 * (1 + np.sign(t_stat) * np.sqrt(1 - np.exp(-2*t_stat**2/np.pi))))
            else:
                # For larger t with small df, be more conservative
                p_value = min(0.1, 2 * (1 - 0.5 * (1 + np.sign(t_stat) * np.sqrt(1 - np.exp(-2*t_stat**2/np.pi)))))
...
def compute_cross_task_subspace(task_activations_dict, variance_threshold=0.95,
                                min_dim=1, max_dim=2000, return_full_pca=False):
    """
...
class JointSubspaceRemovalHook:
    ...
    def __init__(self, layer_idx, joint_subspace, enabled=True, track_stats=False, eps=1e-6,
                 preserve_statistics=True, strength=1.0):
    ...
    def __call__(self, module, input, output):
        ...
        # 计算投影到联合子空间
        if self.preserve_statistics:
            # 当前实现：使用局部标准化（每批次激活的均值和标准差）
            # 这确保了激活分布保持不变，只有子空间成分被移除
            # 注意：子空间是在全局标准化数据上计算的，这里使用局部标准化
            # 这是一个微妙的差异，但对于测量目的应该是合理的

            projection_coeffs = hidden_normalized @ U_orth  # [batch*seq, k]
            # 计算联合子空间成分
            joint_subspace_component = projection_coeffs @ U_orth.T
            # 按强度移除联合子空间成分 (strength=0.0表示不移除，strength=1.0表示完全移除)
        ...
        for i in tqdm(range(0, len(valid_texts), BATCH_SIZE), desc="Computing metrics"):
            ...
            try:
                ...
                if hasattr(outputs, 'logits'):
                    ...
                    # We predict token i+1 from tokens 0 to i
                    ...
                    # Create mask for valid positions (non-padding and within attention mask)
                    attention_mask_shifted = attention_mask[..., 1:].contiguous()  # [batch_size, seq_len-1]
                    valid_mask = attention_mask_shifted.bool()  # Only positions that were attended to

                    # Also exclude pad tokens from accuracy calculation
                    pad_token_id = tokenizer.pad_token_id if tokenizer.pad_token_id is not None else tokenizer.eos_token_id
                    non_pad_mask = (shift_labels != pad_token_id)  # Don't count padding tokens

                    # Combine masks: valid attention positions AND non-padding labels
...
def run_cross_task_experiment_fast_55(model_name, datasets):
    ...
    cross_collector = CrossTaskActivationCollector(
        valid_layers, 
    ...
    for dataset_name, texts in datasets.items():
        ...
        # Calculate optimal samples for activation collection
        ...
        print(f"  Using {len(eval_texts)} samples (constraint satisfied: {constraint_satisfied}, {len(eval_texts)}×{MAX_SEQ_LEN}={len(eval_texts)*MAX_SEQ_LEN} > {HIDDEN_DIM})")
    ...
    for dataset_name in datasets.keys():
        ...
        for layer_idx in valid_layers:
            ...
            if acts is not None and acts.shape[0] > 0:
                ...
                if n_samples < optimal_samples:
                    print(f"  Warning: {dataset_name} only has {acts.shape[0]} activations (need {optimal_samples} for optimal PCA)")
        ...
        if layer_activations:
            ...
            total_samples = sum(acts.shape[0] for acts in layer_activations.values())
    ...
    print(f"\nCross-task subspace computed: {cross_subspace_dim} dimensions")
    ...
    def select_random_non_shared_basis(task_contributions, shared_indices, num_basis, cross_subspace_dim, task_name, trial_seed=None):
        """
        ...
        # Instead of always selecting the lowest variance ones, select randomly from low-variance ones
    ...
    def test_shared_vs_random_basis_importance(model, tokenizer, eval_texts, cross_task_subspace,
                                              shared_indices, task_contributions, dataset_name,
                                              cross_subspace_dim, full_pca_info=None, sample_size=16, n_trials=3):
        """
        ...
        for trial in range(n_trials):
            ...
            try:
                ...
                # Select random non-shared basis vectors for this task (different for each trial)
                ...
                try:
                    ...
                    for layer_idx in valid_layers:
                        ...
                        hook = JointSubspaceRemovalHook(
                            layer_idx, shared_subspace, enabled=True,
                            track_stats=False, preserve_statistics=True
                        )
                        handle = layer.register_forward_hook(hook)
                        shared_hooks.append(handle)

                    shared_metrics = compute_loss(model, tokenizer, eval_subset,
                                                     hooks=shared_hooks, device=DEVICE)
                ...
                try:
                    if full_pca_info and 'components' in full_pca_info:
                        ...
                        if len(available_component_indices) < cross_subspace_dim:
                            print(f"  Warning: Only {len(available_component_indices)} non-shared components available, using all of them")
                            selected_indices = available_component_indices
                        else:
                            # Randomly select same number of components as shared subspace
                            selected_indices = np.random.choice(
                                available_component_indices,
                                size=cross_subspace_dim,
                                replace=False
                            ).tolist()
                        ...
                        print(f"  Random control: Selected {len(selected_indices)} components from PCA space (excluding shared components)")

                        # Verify orthogonality with shared subspace (should be approximately orthogonal)
                        if cross_subspace_dim > 0:
                            ortho_check = np.abs(cross_task_subspace.T @ random_subspace).max()
                            print(f"  Orthogonality check (shared vs random): max overlap = {ortho_check:.6f}")
                    else:
                        # Fallback: Create random orthogonal subspace (old approach)
                        print(f"  Fallback: Creating random subspace (full PCA info not available)")
                        random_matrix = np.random.randn(HIDDEN_DIM, cross_subspace_dim)
                        Q, R = np.linalg.qr(random_matrix)
                        random_subspace = Q.copy()
                    ...
                    for layer_idx in valid_layers:
                        ...
                        layer = layers[layer_idx]
                        hook = JointSubspaceRemovalHook(
                            layer_idx, random_subspace, enabled=True,
                            track_stats=False, preserve_statistics=True
                        )
                        handle = layer.register_forward_hook(hook)
                        random_hooks.append(handle)

                    random_metrics = compute_loss(model, tokenizer, eval_subset,
                                                     hooks=random_hooks, device=DEVICE)
                    ...
                    result.update({
                        ...
                        'random_success': True,
                        'random_note': 'full_space_control'
                    })
        ...
        baseline_perplexities = [r['baseline_perplexity'] for r in trial_results if r is not None and not np.isnan(r['baseline_perplexity'])]
        baseline_accuracies = [r['baseline_accuracy'] for r in trial_results if r is not None and not np.isnan(r['baseline_accuracy'])]

        shared_losses = [r['shared_removal_loss'] for r in trial_results if r and r.get('shared_success')]
        shared_perplexities = [r['shared_perplexity'] for r in trial_results if r and r.get('shared_success') and not np.isnan(r['shared_perplexity'])]
        shared_accuracies = [r['shared_accuracy'] for r in trial_results if r and r.get('shared_success') and not np.isnan(r['shared_accuracy'])]

        random_losses = [r['random_removal_loss'] for r in trial_results if r and r.get('random_success')]
        random_perplexities = [r['random_perplexity'] for r in trial_results if r and r.get('random_success') and not np.isnan(r['random_perplexity'])]
        random_accuracies = [r['random_accuracy'] for r in trial_results if r and r.get('random_success') and not np.isnan(r['random_accuracy'])]
        ...
        if baseline_losses:
            ...
            if is_language_modeling and baseline_perplexities:
                ...
                print(f", Acc: {np.mean(baseline_accuracies)*100:.2f}% ± {np.std(baseline_accuracies)*100:.2f}%", end="")
            print()
        ...
        if shared_losses:
            ...
            print(f"  Shared removal - Loss: {shared_mean:.4f} ± {shared_std:.4f} (impact: +{shared_impact:.2f}%)", end="")
            if is_language_modeling and shared_perplexities and baseline_perplexities:
                ...
                print(f", PPL: {shared_ppl_mean:.2f} ± {shared_ppl_std:.2f} (impact: +{shared_ppl_impact:.2f}%)", end="")
                ...
                print(f", Acc: {shared_acc_mean*100:.2f}% ± {shared_acc_std*100:.2f}% (impact: {shared_acc_impact:+.2f}%)", end="")
            print()
        ...
        if random_losses:
            ...
            print(f"  Random removal - Loss: {random_mean:.4f} ± {random_std:.4f} (impact: +{random_impact:.2f}%)", end="")
            if is_language_modeling and random_perplexities and baseline_perplexities:
                ...
                print(f", PPL: {random_ppl_mean:.2f} ± {random_ppl_std:.2f} (impact: +{random_ppl_impact:.2f}%)", end="")
                ...
                print(f", Acc: {random_acc_mean*100:.2f}% ± {random_acc_std*100:.2f}% (impact: {random_acc_impact:+.2f}%)", end="")
            print()
        ...
        if 'shared_vs_random' in statistical_results:
            ...
            sig = "SIGNIFICANT" if test_result['is_significant'] else "not significant"
            # mean_difference = random_mean - shared_mean
            # If mean_difference < 0: random_mean < shared_mean, so random is better than shared
            # If mean_difference > 0: random_mean > shared_mean, so random is worse than shared
            random_vs_shared = "better" if test_result['mean_difference'] < 0 else "worse"
            print(f"  Shared vs random removal: {sig} (random {random_vs_shared} than shared, p={test_result['p_value']:.4f})")
    ...
    if all_shared_indices:
        ...
        print(f"  占跨任务子空间比例: {len(all_shared_indices)}/{cross_subspace_dim} ({len(all_shared_indices)/cross_subspace_dim*100:.1f}%)")
        ...
        for dataset_name in all_tasks:
            ...
            result = test_shared_vs_random_basis_importance(
                ...
                cross_task_subspace, all_shared_indices,
                task_contributions, dataset_name,
                cross_subspace_dim, full_pca_info=full_pca_info,
                sample_size=16, n_trials=N_STATISTICAL_TRIALS
            )
    ...
    print(f"\n🔬 跨任务子空间:")
    print(f"  - 维度: {cross_subspace_dim}/{4096} ({cross_subspace_dim/4096*100:.1f}%)")
    ...
    if all_shared_indices:
        print(f"\n🌟 共享basis分析:")
        print(f"  - 发现 {len(all_shared_indices)} 个被至少4个任务共享的basis")
        print(f"  - 占总跨任务子空间: {len(all_shared_indices)}/{cross_subspace_dim} ({len(all_shared_indices)/cross_subspace_dim*100:.1f}%)")
        ...
        try:
            ...
            if cross_task_results:
                ...
                if 'cross_subspace_dim' in cross_task_results:
                    ...
                    print(f"\n关键发现:")
                    print(f"  1. 跨任务子空间维度: {cross_dim}/{hidden_dim} ({cross_dim/hidden_dim*100:.1f}%)")
                        ...
                        for task_name, impact in cross_task_results['shared_impacts'].items():
                            if impact.get('statistical_tests'):
                                ...
                                if 'shared_vs_random' in tests:
                                    ...
                                    sig_symbol = "✓" if test_result['is_significant'] else "✗"
                                    random_vs_shared = "better" if test_result['mean_difference'] < 0 else "worse"
                                    print(f"     {task_name}: {sig_symbol} Random significantly {random_vs_shared} than shared (p={test_result['p_value']:.4f})")
                                else:
                                    print(f"     {task_name}: Insufficient data for statistical testing")
                            else:
                                # Fallback to old format
                                shared_impact = impact.get('shared_impact_percent')
                                random_impact = impact.get('random_impact_percent')
                                if shared_impact is not None and random_impact is not None:
                                    diff = shared_impact - random_impact
                                    print(f"     {task_name}: 共享=+{shared_impact:.2f}%, 随机=+{random_impact:.2f}%, 差异={diff:+.2f}%")
                                elif shared_impact is not None:
                                    print(f"     {task_name}: 共享=+{shared_impact:.2f}% (随机测试失败)")
                                else:
