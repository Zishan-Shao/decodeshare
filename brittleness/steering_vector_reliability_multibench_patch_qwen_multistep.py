...
CUDA_VISIBLE_DEVICES=0 python brittleness/steering_vector_reliability_multibench_patch_qwen_multistep.py \
  --model Qwen/Qwen2.5-7B-Instruct \
  --layer 10 --dtype fp16 --device cuda \
  --basis_source multitask \
  --basis_tasks "csqa_pair,arc_challenge_pair,boolq,sst2,strategyqa,piqa" \
  --basis_tasks_per_class 64 \
  --protect_each_step 1 --protect_beta 1.0
...
# -----------------------------
# Prompt encoding (plain vs chat-template)
# -----------------------------
def _truncate_left(input_ids: torch.Tensor, max_len: int) -> torch.Tensor:
...
def encode_prompt_input_ids(
    ...
    *,
    ...
    """
    Returns (1, T) input_ids.

    For Qwen-Instruct (and other instruct/chat models), wrapping the prompt into the
    model's chat template typically improves baseline forced-choice reliability and
    makes steering vectors more consistent.
    """
    if use_chat_template and hasattr(tokenizer, "apply_chat_template") and callable(getattr(tokenizer, "apply_chat_template")):
        messages = []
        if system_prompt:
            messages.append({"role": "system", "content": system_prompt})
        messages.append({"role": "user", "content": prompt_text})
        # add_generation_prompt=True -> includes assistant role prefix, so logits correspond to first generated token.
        ...
        return _truncate_left(ids, max_prompt_tokens)
...
class AddVectorHook:
    """
    Inject alpha*v on seq_len==1 calls (decode steps), with an optional injection window.

    inject_start_step:
      0 = include prompt-boundary step (first generated token)
      1 = skip boundary injection (often makes text less 'glued' in free-form generation)
    inject_first_n:
    ...
    def __init__(
        ...
        *,
        protect_B: Optional[torch.Tensor] = None,
        protect_beta: float = 1.0,
        inject_start_step: int = 0,
        inject_first_n: int = 0,
    ):
        self.v = v.detach()
        self.alpha = float(alpha)
        self.decode_only = decode_only
        self.protect_B = protect_B.detach() if protect_B is not None else None
        self.protect_beta = float(protect_beta)
    ...
    def _B_on(self, device: torch.device, dtype: torch.dtype) -> Optional[torch.Tensor]:
        if self.protect_B is None:
            return None
        key = ("B", device, dtype)
        if key not in self._cache:
            self._cache[key] = self.protect_B.to(device=device, dtype=dtype)
        return self._cache[key]
    ...
    def __call__(self, module, inputs, output):
        ...
        if self.protect_B is not None and abs(self.protect_beta) > 1e-12:
            B = self._B_on(h.device, h.dtype)
            if B is not None:
                # Protect sharedness by ensuring the injected delta lies in B^⊥ at *each decode step*.
                v = v - self.protect_beta * (B @ (B.T @ v))
        h2 = h.clone()
...
def default_candidate_pool(task_name: str) -> List[CandidatePair]:
    # Enforce strict forced-choice for paired MCQ tasks: 1 means option1, 2 means option2.
    # This prevents calibration from selecting ambiguous Yes/No labels.
    ...
    if task_name in ("rte", "qnli"):
        base += [
            ...
            CandidatePair("Entailment/Not", "Entailment", "Not"),
            CandidatePair("entailment/not", "entailment", "not"),
        ]
    if task_name in ("boolq",):
        base += [
            CandidatePair("Correct/Incorrect", "Correct", "Incorrect"),
            CandidatePair("correct/incorrect", "correct", "incorrect"),
        ]
    if task_name in ("piqa",):
        base += [
            CandidatePair("1/2", "1", "2"),
            CandidatePair("A/B", "A", "B"),
            CandidatePair("a/b", "a", "b"),
        ]
    return base
...
def make_piqa_task() -> TaskSpec:
    """
    Physical commonsense (2-choice).
    Map label=True to solution1 being correct (label==0 in HF dataset).
    """
    templates = [
        "Goal: {goal}\n\nSolution 1: {sol1}\nSolution 2: {sol2}\n\nWhich solution is more appropriate?\nAnswer ({POS}/{NEG}): ",
        "Pick the better solution for the goal.\n\nGoal: {goal}\n(1) {sol1}\n(2) {sol2}\n\nAnswer ({POS}/{NEG}): ",
        "Reply with only '{POS}' or '{NEG}'.\n\nGoal: {goal}\nOption 1: {sol1}\nOption 2: {sol2}\n\nAnswer ({POS}/{NEG}): ",
    ]

    def parse_ex(ds, ex):
        goal = ex.get("goal", None)
        s1 = ex.get("sol1", None)
        s2 = ex.get("sol2", None)
        lab = ex.get("label", None)  # 0 or 1
        if goal is None or s1 is None or s2 is None or lab is None:
            return None
        try:
            y = (int(lab) == 0)
        except Exception:
            return None
        return {"goal": str(goal), "sol1": str(s1), "sol2": str(s2)}, bool(y)

    return TaskSpec("piqa", "piqa", None, "train", "validation", templates, parse_ex, default_candidate_pool)
...
def make_strategyqa_task() -> TaskSpec:
    ...
    def parse_ex(ds, ex):
        ...
        if q is None or ans is None:
            return None
        # Some HF copies store answer as string; avoid bool("no")==True.
...
def make_csqa_pair_task() -> TaskSpec:
    ...
    def parse_ex(ds, ex):
        # custom loader populates fields/label already
    ...
    def loader(task: TaskSpec, n_per_class: int, seed: int, split: str) -> List[Dict]:
        ...
        def a_get(ex):
            # answerKey is a label like "A".."E"; map via choices.label
            ch = ex.get("choices", {})
            if not isinstance(ch, dict):
                raise ValueError
            labels = ch.get("label", [])
...
def load_balanced_examples(task: TaskSpec, *, n_per_class: int, seed: int, split: str) -> List[Dict]:
    def _balanced_from_dataset(task2: TaskSpec, ds, *, n_per_class2: int, seed2: int, split_name: str) -> List[Dict]:
        pos_idx, neg_idx = [], []

        for i in range(len(ds)):
            parsed = task2.parse_ex(ds, ds[i])
            if parsed is None:
                continue
            _, y = parsed
        ...
        if len(pos_idx) == 0 or len(neg_idx) == 0:
            raise RuntimeError(f"[{task2.name}] Not enough labeled examples in split={split_name}: pos={len(pos_idx)}, neg={len(neg_idx)}")

        rng = random.Random(seed2)
        rng.shuffle(pos_idx)
        rng.shuffle(neg_idx)
        take = min(n_per_class2, len(pos_idx), len(neg_idx))
        ...
        out = []
        for j in idx:
            parsed = task2.parse_ex(ds, ds[int(j)])
            if parsed is None:
                continue
            fields, y = parsed
    ...
    def _try_splits(splits: List[str]) -> List[Dict]:
        ...
        for sp in splits:
            try:
                if task.custom_loader is not None:
                    return task.custom_loader(task, n_per_class, seed, sp)
                ds = load_dataset(task.ds_path, task.ds_config, split=sp)
                return _balanced_from_dataset(task, ds, n_per_class2=n_per_class, seed2=seed, split_name=sp)
    ...
    # split name fallback for HF datasets that use 'test' instead of 'validation', etc.
...
def compute_raw_margin_pos_minus_neg(
    ...
    *,
    ...
    input_ids = encode_prompt_input_ids(
        tokenizer,
        prompt_text,
    ...
    *,
    ...
    for tid in template_ids:
        for ex in examples:
            ...
            margin = compute_raw_margin_pos_minus_neg(
                model,
                tokenizer,
                prompt,
                pos_ids,
...
def estimate_mean_diff_vector(
    ...
    *,
    ...
    for idx, ex in enumerate(examples):
        ...
        if mode == "prefill":
            h = collect_last_token_state_prefill(
                model,
                tokenizer,
                prompt,
                layer=layer,
            ...
            h = collect_last_token_state_decode_prompt_boundary(
                model,
                tokenizer,
                prompt,
                layer=layer,
...
def estimate_shared_basis_decode(
    ...
    *,
    ...
    if basis_source not in ("neutral", "task"):
        raise ValueError("--basis_source must be 'neutral' or 'task'")
    ...
    X = collect_decode_states_for_prompts(
        model,
        tokenizer,
        prompts,
        layer=layer,
...
def build_multitask_basis_prompts(
    ...
    *,
    ...
    """
    Build prompts from multiple tasks' calibration sets to estimate a shared basis.
    Uses a deterministic candidate pair (first from pool) purely to fill {POS}/{NEG} in templates.
    """
    rng = random.Random(seed)
    prompts: List[str] = []
...
def mean_correct_shift_on_subset(
    ...
    *,
    ...
    use_chat_template: bool,
    system_prompt: str,
    protect_B: Optional[torch.Tensor],
    protect_each_step: bool,
    protect_beta: float,
    inject_start_step: int,
    inject_first_n: int,
) -> float:
    block = get_block(model, layer)
    signs = np.array([1.0 if ex["label"] else -1.0 for ex in examples], dtype=np.float32)

    # Average across templates for more stable sign calibration (especially for chat models).
    base_all = []
    for tid in template_ids:
        base = []
        for ex in examples:
            prompt = format_prompt(task, ex, tid, cand)
            base.append(
                compute_raw_margin_pos_minus_neg(
                    model,
                    tokenizer,
                    prompt,
                    pos_ids,
        ...
        base_all.append(np.array(base, dtype=np.float32) * signs)
    base_corr = np.stack(base_all, axis=0).mean(axis=0)

    if lam == 0.0:
        steered = base_corr.copy()
    else:
        hook = AddVectorHook(
            v=v,
            alpha=lam,
            decode_only=True,
            protect_B=(protect_B if protect_each_step else None),
            protect_beta=protect_beta,
            inject_start_step=inject_start_step,
        ...
        try:
            ...
            for tid in template_ids:
                ...
                steered_all.append(np.array(steered_raw, dtype=np.float32) * signs)
            steered = np.stack(steered_all, axis=0).mean(axis=0)
        finally:
            handle.remove()
    return float((steered - base_corr).mean())
...
def calibrate_sign_and_match_energy(
    ...
    *,
    ...
    ms = mean_correct_shift_on_subset(
        ...
        protect_B=protect_B, protect_each_step=protect_each_step, protect_beta=protect_beta,
        inject_start_step=inject_start_step, inject_first_n=inject_first_n
    )
...
def run_one_task(
    ...
    *,
    ...
    if inject_start_step > 0 and (len(pos_ids) == 1 and len(neg_ids) == 1):
        print(
            f"[Warn][{task.name}] inject_start_step={inject_start_step} skips prompt-boundary injection, "
            "but the candidate pair is single-token. Since this benchmark measures the prompt-boundary "
            "first-token margin, the intervention will have ~0 effect. Use --inject_start_step 0 for this setting."
        )
    ...
    if basis_override is not None:
        ...
        if basis_source == "multitask":
            raise ValueError("basis_source=multitask requires basis_override (computed in main).")
    ...
    v_dec_raw, st_dec = estimate_mean_diff_vector(
        ...
        mode="decode",
        prefill_append_gold_answer=False,
        use_chat_template=use_chat_template,
        system_prompt=system_prompt,
    )
    ...
    sign_tids = [0] if sign_calib_templates == "0" else list(range(len(task.templates)))
    vecs: Dict[str, torch.Tensor] = {}
    decode_variants = []
    for beta in betas:
        ...
        v_beta = calibrate_sign_and_match_energy(
            ...
            layer=layer,
            template_ids=sign_tids,
            v=v_beta_raw,
            target_norm=target_norm,
            max_prompt_tokens=max_prompt_tokens, pos_ids=pos_ids, neg_ids=neg_ids
            ,use_chat_template=use_chat_template, system_prompt=system_prompt,
            protect_B=B, protect_each_step=protect_each_step, protect_beta=protect_beta,
            inject_start_step=inject_start_step, inject_first_n=inject_first_n
        )
...
def main():
    ...
    # Qwen (and other chat models) generally behave better with chat-template prompts.
    ap.add_argument("--use_chat_template", type=int, default=1)
    ap.add_argument(
        "--system_prompt",
        type=str,
        default="You are a helpful assistant. Answer with exactly one of the provided options.",
    )

    ap.add_argument(
        "--tasks",
        type=str,
        default="boolq,sst2,csqa_pair,arc_challenge_pair",
    ...
    ap.add_argument("--basis_source", type=str, default="multitask", choices=["neutral", "task", "multitask"])
    ap.add_argument("--basis_templates", type=str, default="all", choices=["0", "all"])
    ap.add_argument("--basis_max_states", type=int, default=1024)
    ap.add_argument(
        "--basis_tasks",
        type=str,
        default="csqa_pair,arc_challenge_pair,boolq,sst2,strategyqa,piqa",
        help="Comma-separated tasks to build shared basis when --basis_source multitask",
    )
    ap.add_argument(
        "--basis_tasks_per_class",
        type=int,
        default=64,
        help="Per-task per-class examples used to build multitask basis prompts",
    )
    ap.add_argument(
    ...
    # Multistep protection: ensure injected delta lies in B^⊥ at each decode step.
    ap.add_argument("--protect_each_step", type=int, default=1)
    ap.add_argument("--protect_beta", type=float, default=1.0)
    # Pirate-style injection window over decode steps.
    ap.add_argument("--inject_start_step", type=int, default=0, help="0=include prompt-boundary step; 1=skip boundary injection.")
    ap.add_argument("--inject_first_n", type=int, default=0, help="Number of decode steps to inject after start; 0=all.")
    ...
    ap.add_argument("--v_est_templates", type=str, default="all", choices=["0", "all"],
                    help="Estimate v using template 0 only (robustness test) or all templates (more stable).")
    ap.add_argument(
        "--sign_calib_templates",
        type=str,
        default="all",
        choices=["0", "all"],
        help="Calibrate steering sign using template 0 only or average across all templates (more stable).",
    )
    ...
    if args.basis_source == "multitask":
        ...
        for tname in task_names:
            ...
            if per_task:
                ...
                # If user only provided the current task, fall back to using it (avoid empty list crash).
                if len(use_tasks) == 0:
                    use_tasks = basis_task_names
            key = tuple(use_tasks)
            if key not in cache:
    ...
    for tname in task_names:
        ...
        try:
            out = run_one_task(
                ...
                sign_calib_templates=args.sign_calib_templates,
                use_chat_template=bool(args.use_chat_template),
                system_prompt=args.system_prompt,
                protect_each_step=bool(args.protect_each_step),
                protect_beta=float(args.protect_beta),
                inject_start_step=int(args.inject_start_step),
