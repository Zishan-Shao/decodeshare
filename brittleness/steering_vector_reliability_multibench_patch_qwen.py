...
Example:
 CUDA_VISIBLE_DEVICES=0 python brittleness/steering_vector_reliability_multibench_patch_qwen.py \
  --model Qwen/Qwen2.5-7B-Instruct \
  --device cuda --dtype fp32 \
  --layer 10 \
  --tasks strategyqa,csqa_pair,arc_challenge_pair,sst2 \
  ...
  --out_dir brittleness/results/steer_repair_multibench_v3_qwen_mix \
  --show_per_template 1
...
def default_candidate_pool(task_name: str) -> List[CandidatePair]:
    ...
    # This prevents the calibration step from selecting "Yes/No" which is ambiguous here.
    ...
    # Two-choice / multiple-choice-as-binary tasks
    if task_name in ("winogrande", "piqa"):
        base += [
...
def make_winogrande_task() -> TaskSpec:
    """
    Commonsense coreference (2-choice). We map label=True to option1 being correct.
    Dataset: winogrande (config winogrande_xl by default)
    """
    templates = [
        "Choose the correct option to fill the blank.\n\nSentence: {sentence}\n\nOptions:\n1) {option1}\n2) {option2}\n\nAnswer ({POS}/{NEG}): ",
        "Complete the sentence by selecting the best option.\n\n{sentence}\n\n(1) {option1}\n(2) {option2}\n\nAnswer ({POS}/{NEG}): ",
        "Reply with only '{POS}' or '{NEG}'.\n\nSentence: {sentence}\nOption 1: {option1}\nOption 2: {option2}\n\nAnswer ({POS}/{NEG}): ",
    ]

    def parse_ex(ds, ex):
        sent = ex.get("sentence", None)
        o1 = ex.get("option1", None)
        o2 = ex.get("option2", None)
        ans = ex.get("answer", None)  # "1" or "2"
        if sent is None or o1 is None or o2 is None or ans is None:
            return None
        # Standardize blank token used by dataset
        sentence = str(sent).replace("_", "___")
        y = (str(ans).strip() == "1")
        return {"sentence": sentence, "option1": str(o1), "option2": str(o2)}, bool(y)

    # Use validation for eval (train is large); calibration on train.
    return TaskSpec("winogrande", "winogrande", "winogrande_xl", "train", "validation", templates, parse_ex, default_candidate_pool)
...
def make_rotten_tomatoes_task() -> TaskSpec:
    """
    Short movie review sentiment (binary).
    """
    templates = [
        "Review: {text}\n\nIs the sentiment positive?\nAnswer ({POS}/{NEG}): ",
        "Reply with only '{POS}' or '{NEG}'.\nReturn '{POS}' if positive; otherwise '{NEG}'.\n\n{text}\n\nAnswer ({POS}/{NEG}): ",
        "One-word answer: '{POS}' or '{NEG}'.\n\n{text}\n\nSentiment ({POS}/{NEG}): ",
    ]

    def parse_ex(ds, ex):
        txt = ex.get("text", None)
        lab = ex.get("label", None)  # 0/1
        if txt is None or lab is None:
            return None
        if len(str(txt).strip()) < 5:
            return None
        y = (int(lab) == 1)
        return {"text": str(txt)}, bool(y)

    return TaskSpec("rotten_tomatoes", "rotten_tomatoes", None, "train", "validation", templates, parse_ex, default_candidate_pool)


def make_tweet_eval_irony_task() -> TaskSpec:
    """
    Social media irony detection (binary).
    Dataset: tweet_eval/irony
    """
    templates = [
        "Tweet: {text}\n\nIs the tweet ironic?\nAnswer ({POS}/{NEG}): ",
        "Reply with only '{POS}' or '{NEG}'.\nReturn '{POS}' if ironic; otherwise '{NEG}'.\n\n{text}\n\nAnswer ({POS}/{NEG}): ",
        "One-word answer: '{POS}' or '{NEG}'.\n\n{text}\n\nIronic ({POS}/{NEG}): ",
    ]
    ...
    return TaskSpec("tweet_irony", "tweet_eval", "irony", "train", "validation", templates, parse_ex, default_candidate_pool)


def make_tweet_eval_hate_task() -> TaskSpec:
    """
    Social media hate speech detection (binary).
    Dataset: tweet_eval/hate
    """
    templates = [
        "Tweet: {text}\n\nDoes the tweet contain hate speech?\nAnswer ({POS}/{NEG}): ",
        "Reply with only '{POS}' or '{NEG}'.\nReturn '{POS}' if it contains hate speech; otherwise '{NEG}'.\n\n{text}\n\nAnswer ({POS}/{NEG}): ",
        "One-word answer: '{POS}' or '{NEG}'.\n\n{text}\n\nHate speech ({POS}/{NEG}): ",
    ]
    ...
    return TaskSpec("tweet_hate", "tweet_eval", "hate", "train", "validation", templates, parse_ex, default_candidate_pool)
...
TASK_BUILDERS: Dict[str, Callable[[], TaskSpec]] = {
    ...
    # More domains
    "winogrande": make_winogrande_task,          # commonsense coreference (2-choice)
    "piqa": make_piqa_task,                      # physical commonsense (2-choice)
    "rotten_tomatoes": make_rotten_tomatoes_task,  # short reviews (sentiment)
    "tweet_irony": make_tweet_eval_irony_task,   # social media
    "tweet_hate": make_tweet_eval_hate_task,     # social media
}
...
def eval_lambda_sweep(
    ...
    *,
    ...
    # Baseline forced-choice accuracy under the exact evaluation prompts/candidates.
    # Since base_corr is "signed margin" (positive means correct), acc = P(base_corr > 0).
...
def main():
    ...
    ap.add_argument(
        ...
        help="Comma-separated tasks or 'all'. Default: strategyqa,sst2,csqa_pair,arc_challenge_pair. Extra: rte,qnli,imdb,winogrande,piqa,rotten_tomatoes,tweet_irony,tweet_hate,boolq",
    )
    ...
    ap.add_argument(
        ...
        type=int,
        default=32,
        help="Balanced examples per class per basis-task when --basis_source multitask (kept small; basis_max_states is the main cap).",
    )
    ap.add_argument(
