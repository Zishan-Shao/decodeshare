...
One-shot helper to:
  ...
  2) Run `exp_rank_flip.py` on that manifest,
...
def main() -> None:
    ...
    ap.add_argument("--out_dir", type=str, default="", help="Default: outputs/steering_rank_flip/diagnostic_layer<layer>_rand<n>_<ts>/")
    ...
    out_dir = (
        Path(args.out_dir)
        if args.out_dir
        else (repo_root / "outputs" / "steering_rank_flip" / f"diagnostic_layer{args.layer}_rand{args.n_vectors}_{run_id}")
    )
    out_dir.mkdir(parents=True, exist_ok=True)

    dim = int(args.dim)
    if dim <= 0:
        dim = _infer_dim_from_reference(downstream_root) or 4096
