...
def model_input_device(model) -> torch.device:
    ...
    Prefer embedding weights, then hf_device_map, then first non-meta parameter.
    """
    try:
...
def _task_acts_cache_path(cache_dir: str, fold_name: str, task_name: str, layer_idx: int) -> str:
    return os.path.join(
        os.path.expanduser(cache_dir),
        f"acts_{_slugify(fold_name)}__{_slugify(task_name)}__l{int(layer_idx)}.npz",
    )


def _basis_cache_npz_path(cache_dir: str, fold_name: str, layer_idx: int) -> str:
    return os.path.join(os.path.expanduser(cache_dir), f"basis_{_slugify(fold_name)}__l{int(layer_idx)}.npz")


def _basis_cache_meta_path(cache_dir: str, fold_name: str, layer_idx: int) -> str:
    return os.path.join(os.path.expanduser(cache_dir), f"basis_{_slugify(fold_name)}__l{int(layer_idx)}.json")
...
class DecodeLastTokenActivationCollector:
    ...
    Chunks can be either CPU numpy arrays or GPU tensors, depending on `acts_resident`.
    """
    def __init__(self, layer_indices: List[int], acts_resident: str = "cpu"):
        self.layer_indices = list(layer_indices)
        self.acts_resident = str(acts_resident).strip().lower()
        if self.acts_resident not in {"cpu", "gpu"}:
            raise ValueError(f"Unsupported acts_resident={acts_resident!r}; expected 'cpu' or 'gpu'.")
    ...
    def make_hook(self, layer_idx: int):
        def _hook(module, inputs, output):
            ...
            if self.acts_resident == "gpu":
                # Keep activations on-device during collection and materialize to CPU once per task.
...
def compute_shared_subspace_decode_aligned(
    ...
    *,
    ...
    acts_cache_dir: str = "",
    fold_name: str = "",
    resume_acts: bool = True,
    acts_resident: str = "cpu",
    ...
    collector = DecodeLastTokenActivationCollector(layer_indices, acts_resident=acts_resident)
    ...
    try:
        for task_name, prompts in prompts_by_task.items():
            cache_path = _task_acts_cache_path(acts_cache_dir, fold_name, task_name, layer_indices[0]) if acts_cache_dir else ""
            if cache_path and bool(resume_acts) and os.path.exists(cache_path):
                arrs = np.load(cache_path)
                layer_dict = {}
                for layer_idx in layer_indices:
                    key = f"layer_{int(layer_idx)}"
                    if key in arrs:
                        acts = np.asarray(arrs[key], dtype=np.float32)
                        if acts.ndim == 2 and acts.shape[0] > 0:
                            layer_dict[int(layer_idx)] = acts
                            print(f"[Subspace-A3]  loaded cache {task_name} layer={layer_idx}: {acts.shape[0]} x {acts.shape[1]}")
                if layer_dict:
            ...
            if layer_dict:
                ...
                if cache_path:
                    ...
                    np.savez(cache_path, **{f"layer_{int(k)}": v.astype(np.float32) for k, v in layer_dict.items()})
                    print(f"[Subspace-A3]  saved acts cache -> {cache_path}")
            collector.storage.pop(task_name, None)
    finally:
        for h in handles:
            try:
                h.remove()
...
class LastTokenStagedRemovalHook(LastTokenRemovalHook):
    ...
    def __call__(self, module, inputs, output):
        ...
        Q = self._Q(hs.device)
        x = hs[:, -1, :].float()
        if torch.is_tensor(mask) and mask.device != x.device:
...
def run_fold(
    *,
    ...
    basis_cache_dir = str(getattr(args, "basis_cache_dir", "") or "").strip()
    basis_npz = _basis_cache_npz_path(basis_cache_dir, fold_name, layer) if basis_cache_dir else ""
    basis_meta = _basis_cache_meta_path(basis_cache_dir, fold_name, layer) if basis_cache_dir else ""

    if basis_npz and bool(getattr(args, "reuse_basis_cache", True)) and os.path.exists(basis_npz) and os.path.exists(basis_meta):
        arrs = np.load(basis_npz)
        with open(basis_meta, "r", encoding="utf-8") as f:
            basis_payload = json.load(f)
        Q_shared = np.asarray(arrs["Q_shared"], dtype=np.float32)
        Q_rand = np.asarray(arrs["Q_rand"], dtype=np.float32)
        sanity = dict(basis_payload["sanity"])
        extra = dict(basis_payload["extra"])
        kS = int(sanity["shared_basis_dim"])
        cross_dim = int(sanity["cross_dim"])
        er_s = sanity.get("energy_ratio_shared")
        er_r = sanity.get("energy_ratio_rand")
        print(f"[{fold_name}] Loaded basis cache -> {basis_npz}")
    else:
        ...
        joint_subspace, shared_indices, extra, task_acts = compute_shared_subspace_decode_aligned(
            ...
            acts_cache_dir=str(getattr(args, "acts_cache_dir", "") or ""),
            fold_name=fold_name,
            resume_acts=bool(getattr(args, "resume_acts_cache", True)),
            acts_resident=str(getattr(args, "acts_resident", "cpu") or "cpu"),
        )
        ...
        if basis_npz:
            os.makedirs(os.path.dirname(basis_npz), exist_ok=True)
            np.savez(basis_npz, Q_shared=Q_shared.astype(np.float32), Q_rand=Q_rand.astype(np.float32))
            _atomic_json_dump(
                {
                    "fold_name": fold_name,
            ...
            print(f"[{fold_name}] Saved basis cache -> {basis_npz}")
    ...
    if er_s is not None and er_r is not None:
        print(f"[{fold_name}][Sanity] Energy ratio on calib decode states: shared mean={er_s['mean']:.4f}, rand mean={er_r['mean']:.4f}")
    else:
        print(f"[{fold_name}][Sanity] Energy ratio on calib decode states: unavailable in cached sanity metadata")
    ...
    return {
        ...
        "basis": {
            ...
            "shared_indices_count": sanity["shared_basis_dim"],
...
def main():
    ...
    ap.add_argument("--max_memory_map", type=str, default="", help="Explicit max_memory map, e.g. '0:60,1:8,cpu:128'. Overrides --max_memory_per_gpu_gb/--cpu_offload_gb when set.")
    ap.add_argument("--layer", type=int, default=10)
    ap.add_argument("--tasks", type=str, default="gsm8k,commonsenseqa,strategyqa,aqua,arc_challenge,openbookqa,qasc,logiqa")
    ap.add_argument("--mode", type=str, default="loto", choices=["all", "loto"])
    ap.add_argument("--loto_eval_mode", type=str, default="heldout", choices=["heldout", "all"])
    ...
    ap.add_argument("--seed", type=int, default=42)
    ap.add_argument("--sample_seed", type=int, default=12345)
    ap.add_argument("--acts_cache_dir", type=str, default="", help="Optional dir to cache per-fold per-task decode activations for basis estimation.")
    ap.add_argument("--resume_acts_cache", type=int, default=1, choices=[0, 1], help="If 1, reuse cached per-task decode activations when present.")
    ap.add_argument("--acts_resident", type=str, default="cpu", choices=["cpu", "gpu"], help="Where to keep collected decode activations during basis estimation before final concatenation/cache write.")
    ap.add_argument("--basis_cache_dir", type=str, default="", help="Optional dir to save/reuse per-fold shared/random bases.")
    ap.add_argument("--reuse_basis_cache", type=int, default=1, choices=[0, 1], help="If 1, reuse cached per-fold basis when present.")
    ap.add_argument("--resume", type=int, default=1, choices=[0, 1], help="If 1, resume completed held-out folds from out_json when present.")
    ...
    args.use_forced_choice = bool(args.use_forced_choice)
    args.fc_debug_print = bool(args.fc_debug_print)
    args.resume_acts_cache = bool(args.resume_acts_cache)
    args.reuse_basis_cache = bool(args.reuse_basis_cache)
    args.resume = bool(args.resume)
    args.acts_resident = str(args.acts_resident).strip().lower()
    ...
    print(f"[Env] DEVICE={args.device}")
    print(f"[Env] MODEL={args.model} dtype={args.model_dtype}")
    print(f"[Env] device_map={args.device_map!r} max_memory_per_gpu_gb={args.max_memory_per_gpu_gb} cpu_offload_gb={args.cpu_offload_gb} max_memory_map={args.max_memory_map!r} acts_resident={args.acts_resident!r}")
    print(f"[Env] layer_indices={layer_indices}")
    print(f"[Env] tasks={tasks}")
    print(f"[Env] mode={args.mode} loto_eval_mode={args.loto_eval_mode}")
    print(f"[Env] template_randomization={args.template_randomization} shuffle_choices={args.shuffle_choices} add_answer_prefix={args.add_answer_prefix}")
    ...
    model, tokenizer = load_model_and_tokenizer(
        ...
        max_memory_map=args.max_memory_map,
    )
    ...
    results: Dict[str, Any] = {
        "config": {
            ...
            "resume": {
                "acts_cache_dir": args.acts_cache_dir,
                "resume_acts_cache": args.resume_acts_cache,
                "basis_cache_dir": args.basis_cache_dir,
                "reuse_basis_cache": args.reuse_basis_cache,
            },
        }
    }
    ...
    if args.resume and os.path.exists(args.out_json):
        try:
            ...
            if isinstance(prev, dict):
                results.update(prev)
                print(f"[Resume] Loaded existing results from {args.out_json}")
        except Exception as e:
            print(f"[Resume] Failed to load existing results from {args.out_json}: {e}")
    ...
    if args.mode == "all":
        ...
        folds = dict(results.get("folds", {}) or {})
        for holdout in tasks:
            if args.loto_only and holdout != args.loto_only:
                continue
            if args.resume and holdout in folds:
                print(f"[Resume] Skipping completed holdout={holdout}")
                continue
            train_tasks = [t for t in tasks if t != holdout]
            eval_tasks = [holdout] if args.loto_eval_mode == "heldout" else list(tasks)
            fold_name = f"loto_holdout={holdout}"
            ...
            results["folds"] = folds
            _atomic_json_dump(results, args.out_json)
