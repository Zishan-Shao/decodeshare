...
# -----------------------------
# Template randomization (prompt wording)
# -----------------------------
...
def maybe_add_answer_prefix(prompt: str, add_prefix: bool, answer_prefix: str) -> str:
    if not add_prefix:
        return prompt
    # Ensure prefix is at the end and has a leading newline (common for generation)
    if not answer_prefix.startswith("\n"):
        answer_prefix = "\n" + answer_prefix
    return prompt.rstrip() + answer_prefix
...
def shuffle_choices_if_needed(
    ...
    # Re-assign canonical labels A,B,C,... after shuffling to keep parsing simple
    new_labels = MC_LETTERS_8[: len(texts2)]
    # Map old gold -> new gold via position
    old_pos = option_labels.index(gold_label) if gold_label in option_labels else None
    if old_pos is None:
        # fallback: keep as empty; will count as incorrect
        new_gold = ""
    else:
...
# -----------------------------
# Dataset loading (8 tasks)
# -----------------------------
def load_gsm8k_examples(n_subspace: int, n_eval: int, seed: int, tmpl_seed: int, template_randomization: bool, add_answer_prefix: bool, answer_prefix: str):
    ...
    def make_prompt(q: str, ex_id: str) -> str:
        tid = choose_template_id(ex_id, len(GSM8K_TEMPLATES), tmpl_seed) if template_randomization else 0
        p = build_prompt_gsm8k(q, tid)
        return maybe_add_answer_prefix(p, add_answer_prefix, answer_prefix)

    sub_exs, eval_exs = [], []
    for i, ex in enumerate(sub_rows):
        gold = parse_gsm8k_gold(ex["answer"])
        ex_id = f"gsm8k-{sub_split}-{i}"
        sub_exs.append(Example("gsm8k", ex_id, make_prompt(ex["question"], ex_id), gold))
    for i, ex in enumerate(eval_rows):
        gold = parse_gsm8k_gold(ex["answer"])
        ex_id = f"gsm8k-{eval_split}-{i}"
        eval_exs.append(Example("gsm8k", ex_id, make_prompt(ex["question"], ex_id), gold))

    meta = {"hf_id": "gsm8k/main", "subspace_split": sub_split, "eval_split": eval_split}
    return sub_exs, eval_exs, meta

def load_commonsenseqa_examples(n_subspace: int, n_eval: int, seed: int, tmpl_seed: int, template_randomization: bool, shuffle_choices: bool, add_answer_prefix: bool, answer_prefix: str):
    ...
    def make_prompt(q: str, choices: Dict[str, List[str]], gold: str, ex_id: str) -> Tuple[str, str]:
        # Normalize to list of option texts
        labels = [safe_upper(x) for x in choices["label"]]
        texts = [str(x) for x in choices["text"]]
        # If dataset labels aren't A..E, remap to A..E
        if len(labels) != len(texts):
            labels = MC_LETTERS_5[:len(texts)]
        if any(l not in MC_LETTERS_8 for l in labels):
            labels = MC_LETTERS_5[:len(texts)]
            # gold given as A/B/... from dataset; try to map by position if possible
            # But commonsense_qa answerKey is already A-E
        texts2, labels2, gold2 = shuffle_choices_if_needed(texts, labels, safe_upper(gold), shuffle_choices, seed, ex_id)
        tid = choose_template_id(ex_id, len(MC_TEMPLATES), tmpl_seed) if template_randomization else 0
    ...
    sub_exs, eval_exs = [], []
    for i, ex in enumerate(sub_rows):
        ex_id = f"csqa-{sub_split}-{i}"
        p, gold = make_prompt(ex["question"], ex["choices"], ex["answerKey"], ex_id)
        sub_exs.append(Example("commonsenseqa", ex_id, p, gold))
    for i, ex in enumerate(eval_rows):
        ex_id = f"csqa-{eval_split}-{i}"
        p, gold = make_prompt(ex["question"], ex["choices"], ex["answerKey"], ex_id)
        eval_exs.append(Example("commonsenseqa", ex_id, p, gold))

    meta = {"hf_id": "commonsense_qa", "subspace_split": sub_split, "eval_split": eval_split}
    return sub_exs, eval_exs, meta

def load_aqua_examples(n_subspace: int, n_eval: int, seed: int, tmpl_seed: int, template_randomization: bool, shuffle_choices: bool, add_answer_prefix: bool, answer_prefix: str):
    ...
    def make_prompt(q: str, options: List[str], gold: str, ex_id: str) -> Tuple[str, str]:
        # AQUA has 5 options typically, sometimes with leading "A) ..."
        texts = []
    ...
    sub_exs, eval_exs = [], []
    for i, ex in enumerate(sub_rows):
        ex_id = f"aqua-{sub_split}-{i}"
        p, gold = make_prompt(ex["question"], ex["options"], get_gold(ex), ex_id)
        sub_exs.append(Example("aqua", ex_id, p, gold))
    for i, ex in enumerate(eval_rows):
        ex_id = f"aqua-{eval_split}-{i}"
        p, gold = make_prompt(ex["question"], ex["options"], get_gold(ex), ex_id)
        eval_exs.append(Example("aqua", ex_id, p, gold))

    meta = {"hf_id": "aqua_rat", "subspace_split": sub_split, "eval_split": eval_split}
    return sub_exs, eval_exs, meta

def load_strategyqa_examples(n_subspace: int, n_eval: int, seed: int, tmpl_seed: int, template_randomization: bool, add_answer_prefix: bool, answer_prefix: str):
    ...
    def make_prompt(q: str, ex_id: str) -> str:
        tid = choose_template_id(ex_id, len(YN_TEMPLATES), tmpl_seed) if template_randomization else 0
        p = build_prompt_yesno(q, tid)
        return maybe_add_answer_prefix(p, add_answer_prefix, answer_prefix)

    sub_exs, eval_exs = [], []
    for i, ex in enumerate(sub_rows):
        ex_id = f"strategyqa-{sub_split}-{i}"
        sub_exs.append(Example("strategyqa", ex_id, make_prompt(ex["question"], ex_id), to_yesno(ex["answer"])))
    for i, ex in enumerate(eval_rows):
        ex_id = f"strategyqa-{eval_split}-{i}"
        eval_exs.append(Example("strategyqa", ex_id, make_prompt(ex["question"], ex_id), to_yesno(ex["answer"])))

    meta = {"hf_id": hf_id, "subspace_split": sub_split, "eval_split": eval_split, "question_field": "question", "answer_field": "answer"}
    return sub_exs, eval_exs, meta

# ---- Helpers for extra MC datasets (ARC/OpenBookQA/QASC/LogiQA) ----
def _infer_first_key(d: dict, candidates: List[str]) -> Optional[str]:
    for k in candidates:
        if k in d:
            return k
    return None
...
def _extract_question_text(ex: dict) -> str:
    # Common patterns:
    #   arc/qasc: ex["question"] is dict with "stem"
    #   openbookqa: ex["question_stem"]
...
def _extract_choices(ex: dict) -> Tuple[List[str], List[str]]:
    """
    Return (texts, labels) from common schema variants.
    - commonsense/openbook: ex["choices"] dict with lists 'text' + 'label'
    - arc/qasc: ex["question"]["choices"] list of dicts {'text','label'}
    - logiqa: ex["options"] list of strings (no labels)
    """
    # 1) nested under question
    q = ex.get("question", None)
    if isinstance(q, dict) and "choices" in q:
        ch = q["choices"]
        if isinstance(ch, list) and len(ch) > 0 and isinstance(ch[0], dict):
            texts = [str(c.get("text", c.get("content", ""))) for c in ch]
            labels = [safe_upper(c.get("label", MC_LETTERS_8[i])) for i, c in enumerate(ch)]
            return texts, labels
        if isinstance(ch, dict):
            texts = [str(x) for x in ch.get("text", [])]
            labels = [safe_upper(x) for x in ch.get("label", [])]
            return texts, labels
    ...
    if "choices" in ex:
        ...
        if isinstance(ch, dict) and "text" in ch and "label" in ch:
            ...
            labels = [safe_upper(x) for x in ch["label"]]
            return texts, labels
        if isinstance(ch, list) and len(ch) > 0 and isinstance(ch[0], dict):
            texts = [str(c.get("text", c.get("content", ""))) for c in ch]
            labels = [safe_upper(c.get("label", MC_LETTERS_8[i])) for i, c in enumerate(ch)]
    ...
    # 3) openbookqa sometimes uses "choices" but question dict too; handled above

    # 4) logiqa uses "options"
    if "options" in ex and isinstance(ex["options"], (list, tuple)):
        texts = [str(x) for x in ex["options"]]
        labels = MC_LETTERS_8[:len(texts)]
        return texts, labels

    # 5) some datasets use "endings"
    if "endings" in ex and isinstance(ex["endings"], (list, tuple)):
        texts = [str(x) for x in ex["endings"]]
        labels = MC_LETTERS_8[:len(texts)]
        return texts, labels
...
def _extract_answer_key(ex: dict) -> str:
    for k in ["answerKey", "answer_key", "label", "answer", "correct", "gold"]:
        if k in ex:
            v = ex[k]
            # If dict
            if isinstance(v, dict):
                # uncommon; try a typical key
                for kk in ["label", "key", "answerKey", "answer"]:
                    if kk in v:
                        v = v[kk]
                        break
            # int label
            if isinstance(v, (int, np.integer)):
                idx = int(v)
                if 0 <= idx < 8:
                    return MC_LETTERS_8[idx]
            s = safe_upper(v)
            # allow "A", "B", ...; also sometimes "1"/"2"
            if s in MC_LETTERS_8:
                return s
            if s.isdigit():
                j = int(s)
                # Some datasets use 0-based, some 1-based; try both
                if 0 <= j < 8:
                    return MC_LETTERS_8[j]
                if 1 <= j <= 8:
                    return MC_LETTERS_8[j - 1]
            # fallback: extract a letter
            m = re.search(r"\b([A-H])\b", s)
            if m:
                return m.group(1)
    return ""

def _map_gold_label_to_present_labels(gold: str, present_labels: List[str]) -> str:
    g = safe_upper(gold)
    if g in present_labels:
        return g
    # Sometimes labels are "1","2"... or "A","B"...; try mapping A->first
    if g in MC_LETTERS_8 and len(present_labels) > 0:
        idx = MC_LETTERS_8.index(g)
        if idx < len(present_labels):
            return present_labels[idx]
    return ""

def load_arc_challenge_examples(n_subspace: int, n_eval: int, seed: int, tmpl_seed: int, template_randomization: bool, shuffle_choices: bool, add_answer_prefix: bool, answer_prefix: str):
    # ARC has configs; use ARC-Challenge
    try:
    ...
    sub_rows = sample_hf_split(ds[sub_split], n_subspace, seed + 41)
    eval_rows = sample_hf_split(ds[eval_split], n_eval, seed + 42)

    def make_prompt(ex: dict, ex_id: str) -> Tuple[str, str]:
        q = _extract_question_text(ex)
        texts, labels = _extract_choices(ex)
        gold0 = _extract_answer_key(ex)
        gold0 = _map_gold_label_to_present_labels(gold0, [safe_upper(x) for x in labels])
        # re-label canonically A.. for stable parsing
        if len(texts) == 0:
            return "", ""
        canon_labels = MC_LETTERS_8[:len(texts)]
        # if gold0 corresponds to some label in labels, map by position
        if gold0 in [safe_upper(x) for x in labels]:
            pos = [safe_upper(x) for x in labels].index(gold0)
            gold = canon_labels[pos]
        else:
            gold = ""
    ...
    sub_exs, eval_exs = [], []
    for i, ex in enumerate(sub_rows):
        ex_id = f"arc-{sub_split}-{i}"
        p, gold = make_prompt(ex, ex_id)
        sub_exs.append(Example("arc_challenge", ex_id, p, gold))
    for i, ex in enumerate(eval_rows):
        ex_id = f"arc-{eval_split}-{i}"
        p, gold = make_prompt(ex, ex_id)
        eval_exs.append(Example("arc_challenge", ex_id, p, gold))

    meta = {"hf_id": "ai2_arc" + (f"/{cfg}" if cfg else ""), "subspace_split": sub_split, "eval_split": eval_split}
    return sub_exs, eval_exs, meta

def load_openbookqa_examples(n_subspace: int, n_eval: int, seed: int, tmpl_seed: int, template_randomization: bool, shuffle_choices: bool, add_answer_prefix: bool, answer_prefix: str):
    # OpenBookQA has configs; "main" is standard
    try:
    ...
    sub_rows = sample_hf_split(ds[sub_split], n_subspace, seed + 51)
    eval_rows = sample_hf_split(ds[eval_split], n_eval, seed + 52)
    ...
    sub_exs, eval_exs = [], []
    for i, ex in enumerate(sub_rows):
        ex_id = f"openbookqa-{sub_split}-{i}"
        p, gold = make_prompt(ex, ex_id)
        sub_exs.append(Example("openbookqa", ex_id, p, gold))
    for i, ex in enumerate(eval_rows):
        ex_id = f"openbookqa-{eval_split}-{i}"
        p, gold = make_prompt(ex, ex_id)
        eval_exs.append(Example("openbookqa", ex_id, p, gold))

    meta = {"hf_id": "openbookqa" + (f"/{cfg}" if cfg else ""), "subspace_split": sub_split, "eval_split": eval_split}
    return sub_exs, eval_exs, meta

def load_qasc_examples(n_subspace: int, n_eval: int, seed: int, tmpl_seed: int, template_randomization: bool, shuffle_choices: bool, add_answer_prefix: bool, answer_prefix: str):
    ds = load_dataset("qasc")

    sub_split = "train" if "train" in ds else list(ds.keys())[0]
    eval_split = "test" if "test" in ds else ("validation" if "validation" in ds else sub_split)

    sub_rows = sample_hf_split(ds[sub_split], n_subspace, seed + 61)
    eval_rows = sample_hf_split(ds[eval_split], n_eval, seed + 62)
    ...
    sub_exs, eval_exs = [], []
    for i, ex in enumerate(sub_rows):
        ex_id = f"qasc-{sub_split}-{i}"
        p, gold = make_prompt(ex, ex_id)
        sub_exs.append(Example("qasc", ex_id, p, gold))
    for i, ex in enumerate(eval_rows):
        ex_id = f"qasc-{eval_split}-{i}"
        p, gold = make_prompt(ex, ex_id)
        eval_exs.append(Example("qasc", ex_id, p, gold))

    meta = {"hf_id": "qasc", "subspace_split": sub_split, "eval_split": eval_split}
    return sub_exs, eval_exs, meta

def load_logiqa_examples(n_subspace: int, n_eval: int, seed: int, tmpl_seed: int, template_randomization: bool, shuffle_choices: bool, add_answer_prefix: bool, answer_prefix: str):
    # Some hubs expose logiqa as "logiqa" with splits
    ds = load_dataset("logiqa")

    sub_split = "train" if "train" in ds else list(ds.keys())[0]
    eval_split = "test" if "test" in ds else ("validation" if "validation" in ds else sub_split)

    sub_rows = sample_hf_split(ds[sub_split], n_subspace, seed + 71)
    eval_rows = sample_hf_split(ds[eval_split], n_eval, seed + 72)

    def make_prompt(ex: dict, ex_id: str) -> Tuple[str, str]:
        # logiqa sometimes has fields: context, question, options, label
        q = _extract_question_text(ex)
        # include context if available
        ctx = ""
        for k in ["context", "passage", "article", "text"]:
            if k in ex and isinstance(ex[k], str) and len(ex[k]) > 0:
                # Avoid duplicating if question already included
                if ex[k] not in q:
                    ctx = ex[k]
                break
        if ctx:
        ...
        if len(texts) == 0:
            return "", ""
        gold0 = _extract_answer_key(ex)
        # logiqa often uses int label 0-3; handled in _extract_answer_key
        canon_labels = MC_LETTERS_8[:len(texts)]
        # If gold0 is already a canon label, keep. Else try map via present labels.
        gold = gold0 if gold0 in canon_labels else _map_gold_label_to_present_labels(gold0, canon_labels)
        texts2, labels2, gold2 = shuffle_choices_if_needed(texts, canon_labels, gold, shuffle_choices, seed, ex_id)
        tid = choose_template_id(ex_id, len(MC_TEMPLATES), tmpl_seed) if template_randomization else 0
    ...
    sub_exs, eval_exs = [], []
    for i, ex in enumerate(sub_rows):
        ex_id = f"logiqa-{sub_split}-{i}"
        p, gold = make_prompt(ex, ex_id)
        sub_exs.append(Example("logiqa", ex_id, p, gold))
    for i, ex in enumerate(eval_rows):
        ex_id = f"logiqa-{eval_split}-{i}"
        p, gold = make_prompt(ex, ex_id)
        eval_exs.append(Example("logiqa", ex_id, p, gold))

    meta = {"hf_id": "logiqa", "subspace_split": sub_split, "eval_split": eval_split}
    return sub_exs, eval_exs, meta
...
TASK_LOADERS = {
    ...
    "arc_challenge": load_arc_challenge_examples,
    "openbookqa": load_openbookqa_examples,
    "qasc": load_qasc_examples,
    "logiqa": load_logiqa_examples,
}
...
def load_selected_tasks(
    ...
    for name in tasks:
        ...
        # Dispatch based on signature differences
        if name == "gsm8k":
            sub_exs, eval_exs, meta = fn(n_subspace, n_eval, seed, tmpl_seed, template_randomization, add_answer_prefix, answer_prefix)
        elif name == "strategyqa":
            sub_exs, eval_exs, meta = fn(n_subspace, n_eval, seed, tmpl_seed, template_randomization, add_answer_prefix, answer_prefix)
        else:
            # MC tasks
            sub_exs, eval_exs, meta = fn(n_subspace, n_eval, seed, tmpl_seed, template_randomization, shuffle_choices, add_answer_prefix, answer_prefix)
        sub_by[name] = sub_exs
...
def extract_final_answer_line(text: str) -> str:
    """
    Robustly extract the content after 'Final answer:'.
    """
    if not text:
        return ""
        ...
        if m is not None:
            ...
            for j in range(i + 1, len(lines)):
                ...
                if nxt:
                    return nxt
            return ""
    m2 = re.search(r"Final answer\s*:\s*([^\r\n]+)", text, flags=re.IGNORECASE)
    if m2:
...
def parse_prediction(dataset: str, continuation_text: str) -> str:
    ...
    # MC tasks (allow up to H)
    m = re.search(r"\b([A-H])\b", ans_line.upper())
    if m:
        return m.group(1)
    m2 = re.search(r"\b([A-H])\b", continuation_text.upper())
