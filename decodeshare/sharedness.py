...
Run example:
  CUDA_VISIBLE_DEVICES=0 python -m decodeshare.sharedness \
...
# Main
# -----------------------------
def main(default_output_dir: Optional[str] = None):
    output_dir = default_output_dir or os.getcwd()
    default_out_json = os.path.join(output_dir, "sharedness_existence.json")
    ...
    try:
        ...
        if args.tasks != "all":
            ...
            if missing:
                raise RuntimeError(
                    f"--tasks contains unknown tasks: {missing}. Available: {list(prompts_by_task.keys())}"
                )
