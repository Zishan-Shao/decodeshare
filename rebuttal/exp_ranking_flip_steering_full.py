...
"""
exp_ranking_flip_steering_full.py
...
What it does:
  ...
  - Calls `rebuttal/exp_ranking_flip_steering.py` with recommended multi-template args.
  - Writes outputs under a single output directory.

Examples:
  # Auto-discover vectors (from known Llama-2-7B steering-vector dirs) and run:
  python rebuttal/exp_ranking_flip_steering_full.py

  # Use your own manifest (recommended if you have >=50 layer-28 vectors):
  python rebuttal/exp_ranking_flip_steering_full.py --vectors_manifest /path/to/your_vectors.jsonl
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        type=str,
        default="",
        help="If empty: results/rebuttal_<timestamp>/ (under repo root).",
    )
    ...
    if layers:
        out_root = Path(args.out_dir) if args.out_dir else (repo_root / "results" / f"rebuttal_rankflip_sweep_{run_id}")
        ...
        for model_name in models:
            ...
            dim = int(args.dim) if int(args.dim) > 0 else (_infer_dim_from_reference(repo_root) or 4096)
    ...
    # -------------------------
    out_dir = Path(args.out_dir) if args.out_dir else (repo_root / "results" / f"rebuttal_rankflip_full_{run_id}")
