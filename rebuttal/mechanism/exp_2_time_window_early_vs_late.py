# -*- coding: utf-8 -*-
"""
exp_2_time_window_early_vs_late.py
...
Typical run
-----------
CUDA_VISIBLE_DEVICES=7 python rebuttal/mechanism/exp_2_time_window_early_vs_late.py \\
  ...
  --window_n 4 --out_dir results/rebuttal_mechanism/m2_time_window_l28
"""
...
def main() -> None:
    ...
    out_json = os.path.join(out_dir, f"exp_2_time_window_layer{int(args.layer)}{tag}.json")
    _atomic_json_dump(out, out_json)

    # Render MD summary
    cond_order = ["baseline", "early1", key_early, key_late, "full"]
    ...
    md: List[str] = []
    md.append("# Exp-2 (M2): Early-decode vs late-decode window localization")
    md.append("")
    ...
    md.append("## Effect localization (macro, early/late vs full)")
    md.append("```json")
    ...
    md_path = os.path.join(out_dir, f"exp_2_time_window_layer{int(args.layer)}{tag}.md")
    _atomic_text_dump("\n".join(md).rstrip() + "\n", md_path)
