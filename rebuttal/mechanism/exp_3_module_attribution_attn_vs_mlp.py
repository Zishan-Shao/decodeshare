# -*- coding: utf-8 -*-
"""
exp_3_module_attribution_attn_vs_mlp.py

Mechanism experiment M3-a (minimal "attention vs MLP" attribution):
  Causal location test inside a single block, with a late-window target + placebo window.

Core idea
---------
Run the *same* decode-shared basis Q at two locations (attention vs MLP), but only within
specific decode-time windows:

  - lateN: aligned with Exp-2 (answer-entry stage)
  - midN : placebo mid window (should look like baseline)

Locations (within layer ℓ):
  (1) `attn_out`: after attention residual-add, before MLP
  (2) `mlp_out` : after MLP residual-add (layer output)

All else is held fixed. We compare which location better reproduces the previously
observed "brittleness reduction" (template-variance decrease / worst-case improvement).

Design notes
------------
- Decode-only intervention: applies only when seq_len == 1 (KV-cached decode).
- Forced-choice logprob evaluation with teacher-forced warmup so there is a meaningful
  "decode trajectory" rather than a single-step boundary.
- Optional: exclude the final pre-scoring decode step to avoid a trivial direct-logit effect
  at the scoring boundary (recommended; default on).

Typical run
-----------
CUDA_VISIBLE_DEVICES=0 python rebuttal/mechanism/exp_3_module_attribution_attn_vs_mlp.py \\
  ...
  --window_n 4 --add_mid_window 1 \\
  --exclude_final_step 1 \\
  --out_dir results/rebuttal_mechanism/m3_attn_vs_mlp_l28


python rebuttal/mechanism/exp_3_module_attribution_attn_vs_mlp.py --model meta-llama/Llama-2-7b-chat-hf --device cuda --dtype fp16 --layer 28 --alpha_remove 1.0 --basis_npz results/rebuttal_mechanism/logit_lens_l28/basis_layer28_tseed1234.npz --k_basis 32 --tasks commonsenseqa,arc_challenge,openbookqa,qasc,logiqa,boolq --eval_n 128 --template_seeds 1234,2345,3456 --fc_warmup_tokens 32 --fc_prefix_mode auto --fc_answer_prefix $'\\nFinal answer:' --window_n 4 --add_mid_window 1 --out_dir results/rebuttal_mechanism/m3_attn_vs_mlp_l28
"""
...
# -----------------------------------------------------------------------------
# Location-specific decode-only removal hooks
# -----------------------------------------------------------------------------
...
class WindowedLastTokenRemovalPreHook(_BaseWindowedHook):
    """
    Forward *pre* hook: remove projection onto Q from the module *input* (hidden_states)
    on decode steps only.

    Important: we modify the input tensor *in-place* so that, in Llama-like blocks,
    the "residual" reference used later in the block is also affected. This makes the
    intervention match "remove after attention output (post-residual-add)" more closely.
    """
    ...
    def __call__(self, module, inputs):
        ...
        x = hs[:, -1, :].float()
        proj = (x @ Q) @ Q.T
        hs[:, -1, :] = (x - self.alpha * proj).to(dtype=hs.dtype)
...
def _find_post_attn_norm(layer) -> Tuple[Any, str]:
    """
    Return a module whose input corresponds to "hidden_states after attention residual add".

    For Llama/Mistral/Gemma-like HF decoder layers this is typically `post_attention_layernorm`.
    """
    for attr in [
        ...
        if hasattr(layer, attr):
            ...
            if isinstance(mod, torch.nn.Module):
                return mod, str(attr)
    raise RuntimeError(
        "Could not find a post-attention norm module on this layer. "
        "Expected an attribute like `post_attention_layernorm` (Llama-like models)."
    )
...
def _register_location_hook(
    ...
    *,
    ...
    """
    location:
      - "mlp_out": forward hook on the layer output (post-MLP residual add)
      - "attn_out": pre-hook on post-attention norm input (post-attn residual add, pre-MLP)
    """
    if Q_np is None:
    ...
    if loc == "mlp_out":
        stats = WindowHookStats(name, location="mlp_out", module_ref=f"layers[{int(layer_idx)}]")
    ...
    if loc == "attn_out":
        mod, attr = _find_post_attn_norm(layer)
        stats = WindowHookStats(name, location="attn_out", module_ref=f"layers[{int(layer_idx)}].{attr}")
        hk = WindowedLastTokenRemovalPreHook(Q_np, float(alpha), int(window_start), int(window_end), stats)
        handle = mod.register_forward_pre_hook(hk)
        return [handle], hk, stats

    raise ValueError(f"Unknown location={location!r} (expected 'attn_out' or 'mlp_out').")
...
def main() -> None:
    ...
    # Window toggle (recommended on; matches exp_2)
    ap.add_argument(
        "--exclude_final_step",
        type=int,
        default=1,
    ...
    # Output
    ap.add_argument("--out_dir", type=str, default="results/rebuttal_mechanism/m3_attn_vs_mlp")
    ...
    # Attribution summary (focus on brittleness proxies: macro std/range; lower is better)
    ...
    base = f"exp_3_attn_vs_mlp_late{Nw}" + (f"_mid{Nw}" if bool(args.add_mid_window) else "")
    ...
    md: List[str] = []
    md.append("# Exp-3 (M3-a): Module attribution (attn_out vs mlp_out; late window + placebo)")
    md.append("")
    md.append("Intervention locations:")
    md.append("- `attn_out`: pre-hook at post-attention norm input (after attention residual-add, before MLP)")
    md.append("- `mlp_out` : forward hook at layer output (after MLP residual-add)")
    md.append("")
