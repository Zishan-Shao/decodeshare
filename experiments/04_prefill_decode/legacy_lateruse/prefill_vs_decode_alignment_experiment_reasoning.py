...
This code should focus on the reasoning ability of the model. See if it is impaired once 
when the shared basis is removed. Basically with forced-choice logprob evaluation. (I think we should change this to
something like gold standard entropy evaluation something like this I forgot?)

Prefill-vs-Decode shared-basis *alignment* experiment (estimation distribution mismatch).

This variant:
  1) Supports the 9-benchmark task set used in your energy_balance_loto8 script:
        gsm8k, commonsenseqa, strategyqa, aqua, arc_challenge,
        openbookqa, qasc, logiqa, boolq

  2) Uses the unified HF benchmark loaders from `benchmark_dataloaders.py`
     (instead of importing per-task loaders from the LOTO script).
     This removes dependency on:
        disturb_CoT_shared_acc_lasttoken_fp32_sanity_energy_balance_loto8.py

  3) Is robust across common HF causal LM families (LLaMA, Qwen, Gemma, ...):
        - works with/without pad_token
        - optional --trust_remote_code for models that need it
        - layer index sanity checks

Evaluation protocol:
  - Default: forced-choice logprob (decode-aligned) for tasks with discrete candidates
             (all tasks except gsm8k).
  - gsm8k is always evaluated with free-form generation + parsing.
  - If you pass --do_generation 1, all tasks use free-form generation evaluation
    (parsing "Final answer: ...").

Warmup-forced-choice (v3):
  - Optional baseline warmup tokens W are generated once and teacher-forced for all conditions,
    moving the decision point deeper into decode distribution.
  - Optional teacher-forced answer prefix (default "\\nFinal answer:") before scoring candidates.

NOTE about answer prefixes:
  - Dataset prompts can optionally be built with --add_answer_prefix 1 (via benchmark_dataloaders).
  - Forced-choice also has its own teacher-forced prefix via --fc_add_answer_prefix / --fc_answer_prefix.
  - Avoid double-prefixing by setting one of them to 0 if you enable the other.

Example:

# sanity check with warmup = 0
CUDA_VISIBLE_DEVICES=0 python prefill_vs_decode_alignment_experiment_reasoning_fixed.py \
  --model meta-llama/Llama-2-7b-chat-hf --device cuda --dtype fp32 \
  --layer 10 --n_prompts 256 --eval_n 512 \
  --do_generation 0 \
  --fc_warmup_tokens 0 \
  --fc_prefix_mode auto \
  --fc_answer_prefix $'\nFinal answer:' \
  --out_json "results/check/llama2_7b_prefill_vs_decode_alignment_9tasks_fc_reasoning.json" \
  --out_txt "results/check/llama2_7b_prefill_vs_decode_alignment_9tasks_fc_reasoning.txt" \
  --out_md "results/check/llama2_7b_prefill_vs_decode_alignment_9tasks_fc_reasoning.md" \
  --out_tex "results/check/llama2_7b_prefill_vs_decode_alignment_9tasks_fc_reasoning.tex"
...
CUDA_VISIBLE_DEVICES=0 python prefill_vs_decode_alignment_experiment_reasoning_fixed.py \
  --model meta-llama/Llama-2-7b-chat-hf --device cuda --dtype fp32 \
  --layer 10 --n_prompts 256 --eval_n 1024 \
  --do_generation 0 \
  --fc_warmup_tokens 128 \
  --fc_prefix_mode auto \
  --fc_answer_prefix $'\nFinal answer:' \
  --out_json results/fixed_w128.json --out_md results/fixed_w128.md \
  --out_json "results/prefill_decode/llama2_7b_prefill_vs_decode_alignment_9tasks_fc_reasoning.json" \
  --out_txt "results/prefill_decode/llama2_7b_prefill_vs_decode_alignment_9tasks_fc_reasoning.txt" \
  --out_md "results/prefill_decode/llama2_7b_prefill_vs_decode_alignment_9tasks_fc_reasoning.md" \
  --out_tex "results/prefill_decode/llama2_7b_prefill_vs_decode_alignment_9tasks_fc_reasoning.tex"
...
from datasets import load_dataset  # keep for potential downstream usage / HF init
...
try:
    from decodeshare.benchmark_dataloaders import (  # noqa: E402
        ...
        is_correct as is_correct_generation,
    )
...
#TASK_DEFAULT_9 = "gsm8k,commonsenseqa,strategyqa,arc_challenge,openbookqa,qasc,logiqa,boolq"
...
def load_selected_tasks_9(
    ...
    *,
    ...
    # Match the previous verbose per-task prints
    for name in tasks:
...
def candidate_strings(task: str) -> List[str]:
    ...
    IMPORTANT: These must match the gold labels produced by benchmark_dataloaders.
      - Most MC tasks => letters.
      - strategyqa => Yes/No strings (gold is YES/NO).
    """
    ...
    *,
    ...
    try:
        for i in tqdm(range(0, len(prompts), batch_size), desc=f"ForcedChoice({task})"):
            ...
            # Teacher-force answer prefix before scoring candidates (recommended)
            if answer_prefix:
    ...
    *,
    ...
    try:
        ...
        for i, (cont, gold) in enumerate(zip(conts, golds)):
            ...
            correct[i] = 1.0 if is_correct_generation(task, pred, gold) else 0.0
...
def load_model_and_tokenizer(model_name: str, device: str, dtype: str, trust_remote_code: bool = False):
    ...
    tok.padding_side = "left"
    if tok.pad_token is None:
        # Some tokenizers (LLaMA/Gemma) lack pad_token by default.
        tok.pad_token = tok.eos_token
...
def main():
    ...
    ap.add_argument("--trust_remote_code", type=int, default=0, choices=[0, 1], help="Set 1 for models that require trust_remote_code (some Qwen variants, etc.).")
    ...
    ap.add_argument("--do_generation", type=int, default=0, choices=[0, 1], help="If 1, evaluate ALL tasks via free-form generation (parsing). Otherwise, use forced-choice for MC tasks.")
    ap.add_argument("--match_state_count", type=int, default=0, choices=[0, 1], help="If 1, match decode/prefill state counts for basis estimation.")
    ...
    ap.add_argument("--fc_add_answer_prefix", type=int, default=1, choices=[0, 1])
    ...
    ap.add_argument("--out_json", type=str, default=os.path.join(THIS_DIR, "prefill_vs_decode_alignment_9tasks.json"))
    ap.add_argument("--out_txt", type=str, default=os.path.join(THIS_DIR, "prefill_vs_decode_alignment_9tasks.txt"))
    ap.add_argument("--out_md", type=str, default=os.path.join(THIS_DIR, "prefill_vs_decode_alignment_9tasks.md"))
    ap.add_argument("--out_tex", type=str, default=os.path.join(THIS_DIR, "prefill_vs_decode_alignment_9tasks.tex"))
    ...
    for t in tasks:
        if t not in TASKS_9:
            raise ValueError(f"Unknown task '{t}'. Supported for this script: {sorted(TASKS_9)}")
    ...
    if not bool(args.do_generation):
        ...
        summary_lines.append(f"Forced-choice add answer prefix: {bool(args.fc_add_answer_prefix)} prefix={args.fc_answer_prefix!r}")
    else:
