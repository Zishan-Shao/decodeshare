...
def _normalize_ci(ci: Any, *, is_prob: bool) -> Optional[Tuple[float, float]]:
    ...
    # normalize percent->prob if needed
...
def run_signature(cfg: Dict[str, Any]) -> str:
    # your JSON configs differ across scripts; be tolerant
...
def summarize_one_file(
    ...
    *,
    ...
    # flag counts (by flag) + flagged task counts (by condition)
    ...
    # Flag summary
    flag_header = ["Condition", "Total flagged tasks", "PARSE↓", "PARSE low", "EOS shift", "LEN↓"]
    ...
    md.append(
        f"- Heuristics: baseline_extr≥{require_base_extr:.2f} & extr_drop≥{parse_drop_thresh:.2f} → `PARSE↓`; "
        f"extr≤{extr_floor:.2f} → `PARSE low`; |Δeos|≥{eos_delta_thresh:.2f} → `EOS shift`; "
        f"len_ratio≤{len_ratio_thresh:.2f} → `LEN↓`\n\n"
    )
    md.append(render_md_table(flag_header, flag_rows))
    md.append("\n")

    md.append("### Extraction rate per task (%, with CI if available; Δ vs baseline)\n")
    ...
    md.append("### EOS rate per task (%, with CI if available; Δ vs baseline)\n")
    md.append(render_md_table(
    ...
    md.append("### Avg new tokens per task (with CI if available; Δ and ratio vs baseline)\n")
    md.append(render_md_table(
        ["Task", "n", "Baseline", "Shared(full)", "Shared(staged)", "Rand(full)", "Rand(staged)"],
        rows_len
    ))
    ...
    if include_accuracy:
        md.append("### Accuracy per task (%, with CI if available; Δ vs baseline) [context]\n")
        md.append(render_md_table(
            ["Task", "n", "Baseline", "Shared(full)", "Shared(staged)", "Rand(full)", "Rand(staged)"],
            rows_acc
        ))
...
# Diagnostic summary for disturb_CoT_shared_acc_lasttoken_fp32_sanity_energy_balance_loto8 results.

# What it summarizes (per task):
#   - extraction_rate
#   - eos_rate
#   - avg_new_tokens
#   - (optionally) accuracy for context
#   - flags for suspected "parse failure" / "output behavior shift"

# Supports both:
#   - mode=all: results["all_tasks"]["by_dataset"]
#   - mode=loto: results["folds"][holdout]["by_dataset"][holdout]  (heldout evaluation)
#     If loto_eval_mode="all", it will summarize all datasets inside each fold.
...
#     if delta is None:
#         return "N/A"
#     # delta in proportion space -> percentage points
#     return f"{delta * 100:+.1f}pp"

# def fmt_delta(x: Optional[float], x0: Optional[float], kind: str) -> str:
#     """
#     kind:
#       - "pp" : show percentage-point delta (x - x0) * 100
#       - "raw": show raw delta (x - x0)
#       - "ratio": show x/x0
#     """
#     if x is None or x0 is None:
#         return "N/A"
...
#         f"mode={cfg.get('mode','?')}",
#         f"loto_eval={cfg.get('loto_eval_mode','?')}",
#         f"layer={cfg.get('layer_indices',['?'])[0] if isinstance(cfg.get('layer_indices'), list) else cfg.get('layer_indices','?')}",
#         f"tau={cfg.get('tau','?')}",
#         f"m={cfg.get('m_shared','?')}",
#         f"tr={int(bool(cfg.get('template_randomization', False)))}",
#         f"sc={int(bool(cfg.get('shuffle_choices', False)))}",
#         f"rand={cfg.get('rand_type','?')}",
#         f"dtype={cfg.get('model_dtype','?')}",
#     ]
#     return " | ".join(parts)
...
#     task_key is either:
#       - task name (mode=all)
#       - fold:heldout or fold:task (mode=loto)
#     """
#     cfg = results.get("config", {})
#     mode = cfg.get("mode", "unknown")
...
#     # fallback: try to find something
#     if "all_tasks" in results:
#         fold = results.get("all_tasks", {})
#         by_dataset = fold.get("by_dataset", {})
#         for task, block in sorted(by_dataset.items()):
...
# def get_run_metrics(block: Dict[str, Any], decoding: str, mode: str) -> Optional[Dict[str, float]]:
#     """
#     mode: baseline/shared_full/shared_staged/rand_full/rand_staged
...
#         "extraction_rate": r.get("extraction_rate", None),
#         "eos_rate": r.get("eos_rate", None),
#         "avg_new_tokens": r.get("avg_new_tokens", None),
#     }
...
#     """
#     Heuristic flags:
#       - PARSE↓ : extraction drops a lot from baseline
#       - PARSE low: extraction below a floor
#       - EOS shift: eos_rate changes a lot
#       - LEN↓ : avg_new_tokens shrinks a lot (ratio)
#     """
#     flags: List[str] = []
...
#     # Per-task extraction table rows
#     rows_ex = []
#     rows_eos = []
#     rows_len = []
#     rows_acc = []

#     # Flag counters per condition (vs baseline)
#     flag_counts = {c: defaultdict(int) for c in ["shared_full", "shared_staged", "rand_full", "rand_staged"]}
...
#         base = get_run_metrics(block, decoding, "baseline") or {}
#         base_ex = base.get("extraction_rate", None)
#         base_eos = base.get("eos_rate", None)
#         base_len = base.get("avg_new_tokens", None)
#         base_acc = base.get("accuracy", None)

#         # store baseline into agg
#         if base_acc is not None: agg["baseline"]["acc"].append(float(base_acc))
#         if base_ex is not None:  agg["baseline"]["ex"].append(float(base_ex))
#         if base_eos is not None: agg["baseline"]["eos"].append(float(base_eos))
#         if base_len is not None: agg["baseline"]["len"].append(float(base_len))

#         # helper to format one condition cell
#         def cond_cell(metric_name: str, cond_name: str, kind: str) -> str:
#             m = get_run_metrics(block, decoding, cond_name)
#             if m is None:
#                 return "N/A"
#             v = m.get(metric_name, None)
...
#                 main = fmt_len(v)
#                 d = fmt_delta(v, v0, "raw")
#                 r = fmt_delta(v, v0, "ratio")
#                 return f"{main} ({d}, {r})"
#             return "N/A"

#         # compute cond metrics + flags
#         flags_sf, flags_ss, flags_rf, flags_rs = [], [], [], []
#         for c in ["shared_full", "shared_staged", "rand_full", "rand_staged"]:
#             m = get_run_metrics(block, decoding, c)
#             if m is None:
#                 continue
...
#             # add to agg
#             if m.get("accuracy", None) is not None: agg[c]["acc"].append(float(m["accuracy"]))
#             if m.get("extraction_rate", None) is not None: agg[c]["ex"].append(float(m["extraction_rate"]))
#             if m.get("eos_rate", None) is not None: agg[c]["eos"].append(float(m["eos_rate"]))
#             if m.get("avg_new_tokens", None) is not None: agg[c]["len"].append(float(m["avg_new_tokens"]))
...
#         # Tables (metric-specific)
#         rows_ex.append([
#             task, n_str,
#             fmt_pct(base_ex),
#             cond_cell("extraction_rate", "shared_full", "pp"),
#             cond_cell("extraction_rate", "shared_staged", "pp"),
#             cond_cell("extraction_rate", "rand_full", "pp"),
#             cond_cell("extraction_rate", "rand_staged", "pp"),
#             f"SF:{flags_str(flags_sf)} SS:{flags_str(flags_ss)} RF:{flags_str(flags_rf)} RS:{flags_str(flags_rs)}",
#         ])
#         rows_eos.append([
#             task, n_str,
#             fmt_pct(base_eos),
#             cond_cell("eos_rate", "shared_full", "pp"),
#             cond_cell("eos_rate", "shared_staged", "pp"),
#             cond_cell("eos_rate", "rand_full", "pp"),
#             cond_cell("eos_rate", "rand_staged", "pp"),
#         ])
#         rows_len.append([
#             task, n_str,
#             fmt_len(base_len),
#             cond_cell("avg_new_tokens", "shared_full", "raw"),
#             cond_cell("avg_new_tokens", "shared_staged", "raw"),
#             cond_cell("avg_new_tokens", "rand_full", "raw"),
#             cond_cell("avg_new_tokens", "rand_staged", "raw"),
#         ])
...
#                 task, n_str,
#                 fmt_pct(base_acc),
#                 cond_cell("accuracy", "shared_full", "pp"),
#                 cond_cell("accuracy", "shared_staged", "pp"),
#                 cond_cell("accuracy", "rand_full", "pp"),
#                 cond_cell("accuracy", "rand_staged", "pp"),
#             ])
...
#             mean_or_na(ex),
#             mean_or_na(eos),
#             f"{(sum(ln)/len(ln)):.1f}" if ln else "N/A",
#         ])
...
#     for c in ["shared_full", "shared_staged", "rand_full", "rand_staged"]:
#         cnts = flag_counts[c]
#         total = sum(cnts.values())  # counts flags, not tasks; still useful
#         flag_rows.append([
#             c,
#             str(total),
#             str(cnts.get("PARSE↓", 0)),
...
#     md.append("### Extraction rate per task (%, with Δ vs baseline)\n")
#     md.append(render_md_table(
#         ["Task", "n", "Baseline", "Shared(full)", "Shared(staged)", "Rand(full)", "Rand(staged)", "Flags"],
#         rows_ex
#     ))
...
#     md.append("### EOS rate per task (%, with Δ vs baseline)\n")
#     md.append(render_md_table(
#         ["Task", "n", "Baseline", "Shared(full)", "Shared(staged)", "Rand(full)", "Rand(staged)"],
#         rows_eos
#     ))
...
#     md.append("### Avg new tokens per task (with Δ and ratio vs baseline)\n")
#     md.append(render_md_table(
#         ["Task", "n", "Baseline", "Shared(full)", "Shared(staged)", "Rand(full)", "Rand(staged)"],
#         rows_len
#     ))
...
#     if include_accuracy:
#         md.append("### Accuracy per task (%, with Δ vs baseline) [context]\n")
#         md.append(render_md_table(
#             ["Task", "n", "Baseline", "Shared(full)", "Shared(staged)", "Rand(full)", "Rand(staged)"],
#             rows_acc
#         ))
...
#     # heuristic thresholds
#     ap.add_argument("--parse_drop_thresh", type=float, default=0.20, help="Flag PARSE↓ if baseline_extr>=require_base_extr and extr drops by >= this")
#     ap.add_argument("--extr_floor", type=float, default=0.50, help="Flag PARSE low if extraction_rate <= this")
#     ap.add_argument("--require_base_extr", type=float, default=0.80, help="Only consider PARSE↓ if baseline extraction >= this")
#     ap.add_argument("--eos_delta_thresh", type=float, default=0.20, help="Flag EOS shift if |Δeos| >= this")
#     ap.add_argument("--len_ratio_thresh", type=float, default=0.50, help="Flag LEN↓ if avg_new_tokens ratio <= this")
