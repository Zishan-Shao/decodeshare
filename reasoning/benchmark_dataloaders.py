...
def _is_dataset_cache_metadata_error(e: Exception) -> bool:
    msg = str(e)
    # Some stale local dataset_info.json files cannot be deserialized by the
    # current datasets version. In that case, rebuild the dataset in a fresh
    # cache directory instead of reusing the incompatible cache entry.
    needles = [
...
def _compat_cache_dir(hf_id: str, cfg: Optional[str], revision: Optional[str]) -> str:
    parts = [hf_id.replace("/", "__")]
    if cfg:
        parts.append(cfg.replace("/", "__"))
    if revision:
        parts.append(revision.replace("/", "__").replace(":", "_"))
...
def _load_dataset_compat(*args, **kwargs):
    ...
    In particular, some versions do not accept trust_remote_code.
    """
    try:
...
def load_hf_dataset(
    ...
    **kwargs,
    ...
    Behavior:
      ...
      - If it fails because a stale local dataset cache cannot be deserialized
        by the current datasets version, rebuild in a fresh compat cache dir.
      - Explicitly enforces trust_remote_code=False (with compat fallback).
    """
    if "trust_remote_code" not in kwargs:
        kwargs["trust_remote_code"] = False

    def _call(rev: Optional[str], *, cache_dir: Optional[str] = None, force_redownload: bool = False):
        call_kwargs = dict(kwargs)
        if cache_dir is not None:
            call_kwargs["cache_dir"] = cache_dir
        if force_redownload:
            call_kwargs["download_mode"] = "force_redownload"
        if cfg is None:
            if rev is None:
                return _load_dataset_compat(hf_id, **call_kwargs)
            return _load_dataset_compat(hf_id, revision=rev, **call_kwargs)
        else:
            if rev is None:
                return _load_dataset_compat(hf_id, cfg, **call_kwargs)
            return _load_dataset_compat(hf_id, cfg, revision=rev, **call_kwargs)
    ...
    try:
        ...
        if revision is None and _is_dataset_script_unsupported_error(e):
            try:
                ...
                if _is_dataset_cache_metadata_error(e2):
                    compat_cache_dir = _compat_cache_dir(hf_id, cfg, AUTO_PARQUET_REVISION)
                    print(
                        f"[Warn] Rebuilding stale HF cache for {hf_id}"
                        f"{'' if cfg is None else f'/{cfg}'}"
                        f" @ {AUTO_PARQUET_REVISION} in fresh cache_dir={compat_cache_dir}"
                    )
                    ds = _call(
                        AUTO_PARQUET_REVISION,
                        cache_dir=compat_cache_dir,
                        force_redownload=True,
                    )
                    return ds, AUTO_PARQUET_REVISION
                raise
        if _is_dataset_cache_metadata_error(e):
            compat_cache_dir = _compat_cache_dir(hf_id, cfg, revision)
            print(
                f"[Warn] Rebuilding stale HF cache for {hf_id}"
                f"{'' if cfg is None else f'/{cfg}'}"
                f"{'' if revision is None else f' @ {revision}'}"
                f" in fresh cache_dir={compat_cache_dir}"
            )
            ds = _call(revision, cache_dir=compat_cache_dir, force_redownload=True)
            return ds, revision
        raise
