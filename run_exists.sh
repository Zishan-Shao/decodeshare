...
NULL_SCRAMBLE_TRIALS=200   # 你原来的 TRIALS=100 实际对应这个

mkdir -p results/exists_scramble_more_trials/
        ...
        for CALIB_MAX_NEW_TOKENS in "${CALIB_MAX_NEW_TOKENS_LIST[@]}"; do
          for MAX_PROMPT_LEN in "${MAX_PROMPT_LEN_LIST[@]}"; do
            for PER_TASK_MAX_STATES in "${PER_TASK_MAX_STATES_LIST[@]}"; do
              ...
              OUT_JSON="results/exists_scramble_more_trials/prove_existence_${MODEL_TAG}_fp32_layer${LAYER}_n${N_PROMPTS}_new${CALIB_MAX_NEW_TOKENS}_maxlen${MAX_PROMPT_LEN}_states${PER_TASK_MAX_STATES}_tau${TAU}_msharedall_perm${NULL_PERM_TRIALS}_scr${NULL_SCRAMBLE_TRIALS}.json"
              ...
              echo "[Run] model=${MODEL_NAME} layer=${LAYER} tau=${TAU} n=${N_PROMPTS} new=${CALIB_MAX_NEW_TOKENS} maxlen=${MAX_PROMPT_LEN} states=${PER_TASK_MAX_STATES}"
                ...
                CUDA_VISIBLE_DEVICES=2 python prove_sharedness_decode_fair.py \
                --model "${MODEL_NAME}" \
                --device cuda \
                --model_dtype fp32 \
                --layer "${LAYER}" \
