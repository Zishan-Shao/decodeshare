# -*- coding: utf-8 -*-
"""
openanswer_subspace_patching_qwenfix.py

Fix: support Qwen / chat-model generation with correct HF chat template.

Key fixes vs your original:
1) Add --prompt_format {auto,plain,chat} and --system_prompt.
   - For Qwen Instruct/Chat, you MUST use tokenizer.apply_chat_template(..., add_generation_prompt=True)
     otherwise generation/logprob is mis-templated and unstable.  (Qwen docs / HF model cards)
2) If apply_chat_template(tokenize=False) then tokenize later with add_special_tokens=False
   to avoid duplicating special tokens (HF chat templating docs).
3) Greedy loop stop condition: handle eos_token_id being int OR list/tuple (common for Qwen2.5 variants).
4) greedy generation decodes ONLY the newly generated tokens (assistant response),
   instead of decoding prompt+response together (this avoids math/code evaluation being polluted by prompt).

Your original open-answer patching logic is preserved.

References:
- Qwen docs: apply_chat_template + add_generation_prompt=True adds "<|im_start|>assistant\n"  :contentReference[oaicite:3]{index=3}
- HF Qwen2.5 model cards: recommend latest transformers; use apply_chat_template  :contentReference[oaicite:4]{index=4}
- HF chat templating: if template -> tokenize later, set add_special_tokens=False  :contentReference[oaicite:5]{index=5}
- Qwen eos may be list (e.g. [<|im_end|>, <|endoftext|>])  :contentReference[oaicite:6]{index=6}

🧪 你怎么用它跑 Qwen（对应你 A/B/C/D 的命令）

下面给你一套直接替换成 Qwen 的命令模板（只改必要项：--model + --prompt_format chat + --system_prompt；其余保持你原实验逻辑）。

⚠️ 重要：Qwen2.5 系列要求 transformers 版本足够新；HF 模型卡明确提到 <4.37.0 可能报 KeyError: 'qwen2'

A) GSM8K pair_logprob（主证据）
python openanswer_subspace_patching_qwenfix.py \
  --base_script_path subspace_patching_transfer.py \
  --model Qwen/Qwen2.5-7B-Instruct --device cuda --dtype bf16 \
  --prompt_format chat --system_prompt "You are a helpful assistant." \
  --layer 10 --seed 123 \
  --task gsm8k --n_eval 256 --max_flips 64 \
  --eval_mode pair_logprob \
  --Qs_path Q_shared_layer10.npy \
  --patch_n_steps 4 \
  --out_json results/openanswer_qwen/gsm8k_pairlogprob.json --gold_text_prefix "" --dist_text_prefix ""

B) GSM8K gen_math（附录证据）
python openanswer_subspace_patching_qwenfix.py \
  --base_script_path subspace_patching_transfer.py \
  --model Qwen/Qwen2.5-7B-Instruct --device cuda --dtype bf16 \
  --prompt_format chat --system_prompt "You are a helpful assistant." \
  ...
  --out_json results/openanswer_qwen/gsm8k_genmath.json

C) HumanEval pair_logprob（主证据）
python openanswer_subspace_patching_qwenfix.py \
  --base_script_path subspace_patching_transfer.py \
  --model Qwen/Qwen2.5-Coder-7B-Instruct --device cuda --dtype bf16 \
  --prompt_format chat --system_prompt "You are a helpful assistant." \
  --layer 10 --seed 123 \
  ...
  --gold_max_tokens 128 \
  --patch_n_steps 4 \
  --out_json results/openanswer_qwen/humaneval_pairlogprob.json
...
python openanswer_subspace_patching_qwenfix.py \
  ...
  --out_json results/openanswer_qwen/humaneval_gencode_compile.json
...
def _is_chat_tokenizer(tok: Any) -> bool:
    # chat_template exists for chat/instruct models; Qwen uses it heavily.
    return bool(getattr(tok, "chat_template", None))


@dataclass
class PromptConfig:
    prompt_format: str  # "plain" or "chat"
    system_prompt: str = ""
    add_generation_prompt: bool = True
    add_special_tokens_plain: bool = True  # only used in plain mode
    enable_thinking: Optional[bool] = None  # Qwen3-style, if supported
...
def _tokenize_prompt(
    ...
    *,
    ...
    """
    Returns: (input_ids, attention_mask) on device.
    """
    if cfg.prompt_format == "plain":
        enc = tok(prompt_text, return_tensors="pt", add_special_tokens=bool(cfg.add_special_tokens_plain))
    ...
    # Try safest path: tokenize=True (avoids special token duplication gotchas)
    # If tokenizer doesn't support enable_thinking / return_tensors, fall back.
    kwargs_base: Dict[str, Any] = {"add_generation_prompt": bool(cfg.add_generation_prompt)}
    if cfg.enable_thinking is not None:
        kwargs_base["enable_thinking"] = bool(cfg.enable_thinking)

    # 1) try tokenize=True, return_tensors="pt"
    try:
        enc = tok.apply_chat_template(messages, tokenize=True, return_tensors="pt", **kwargs_base)
    ...
    # 2) retry without enable_thinking if it caused the failure
    kwargs_no_think = dict(kwargs_base)
    kwargs_no_think.pop("enable_thinking", None)
    try:
        enc = tok.apply_chat_template(messages, tokenize=True, return_tensors="pt", **kwargs_no_think)
        if isinstance(enc, torch.Tensor):
            input_ids = enc
            attn = torch.ones_like(input_ids)
        else:
    ...
    # 3) tokenize=True but without return_tensors
    try:
        ids = tok.apply_chat_template(messages, tokenize=True, **kwargs_no_think)
    ...
    # 4) tokenize=False -> then tokenize string with add_special_tokens=False (HF doc warning)
    text = tok.apply_chat_template(messages, tokenize=False, **kwargs_no_think)
    enc2 = tok(text, return_tensors="pt", add_special_tokens=False)
    input_ids = enc2["input_ids"]
    attn = enc2.get("attention_mask", torch.ones_like(input_ids))
    return input_ids.to(device), attn.to(device)


def _get_eos_id_set(model: AutoModelForCausalLM, tok: AutoTokenizer) -> Set[int]:
    """
    Qwen variants sometimes have eos_token_id as a list, so handle all.
    """
    eos: Union[None, int, List[int], Tuple[int, ...]] = None
    try:
...
def decode_aligned_logprob(
    ...
    *,
    ...
    if prompt_cfg is None:
        ...
        input_ids, attn = _tokenize_prompt(tok, prompt, cfg=prompt_cfg, device=device)
...
def greedy_generate_decode_aligned(
    ...
    *,
    ...
    if prompt_cfg is None:
        toks = tok(prompt, return_tensors="pt", add_special_tokens=add_special_tokens_prompt)
        input_ids = toks["input_ids"].to(device)
        attn_full = toks.get("attention_mask", torch.ones_like(input_ids)).to(device)
    else:
        input_ids, attn_full = _tokenize_prompt(tok, prompt, cfg=prompt_cfg, device=device)
    ...
    eos_ids = _get_eos_id_set(model, tok)
    ...
    try:
        ...
        for _ in range(int(max_new_tokens)):
            ...
            if stop_on_eos and eos_ids and next_id in eos_ids:
                stopped_eos = True
                break
        ...
        # IMPORTANT FIX: decode ONLY generated tokens (assistant response),
        # not prompt+response together.
...
def main():
    ...
    # NEW: prompt formatting
    ap.add_argument("--prompt_format", type=str, default="auto", choices=["auto", "plain", "chat"],
                    help="auto: use chat only for Qwen-like models with chat_template; plain: old behavior; chat: force apply_chat_template")
    ap.add_argument("--system_prompt", type=str, default="You are a helpful assistant.")
    ap.add_argument("--enable_thinking", type=int, default=-1,
                    help="For Qwen3-style templates: -1 do not pass; 0/1 pass enable_thinking")
    ...
    ap.add_argument("--add_special_tokens_prompt", type=int, default=1,
                    help="Only used in plain prompt mode. For chat templates it's ignored.")
    ap.add_argument("--trust_remote_code", type=int, default=1, help="Trust remote code for model loading.")
    ...
    # Decide prompt format (auto -> use chat only for Qwen-like models)
    model_name_l = (args.model or "").lower()
    has_chat_template = _is_chat_tokenizer(tok)

    if args.prompt_format == "plain":
        use_chat = False
    elif args.prompt_format == "chat":
        use_chat = True
    else:
        # auto: only enable chat when (a) tokenizer has chat_template AND (b) model looks like Qwen
        use_chat = bool(has_chat_template and ("qwen" in model_name_l))

    if use_chat and not has_chat_template:
        print("[Warn] prompt_format wants chat, but tokenizer.chat_template is empty -> fallback to plain.")
        use_chat = False

    enable_thinking: Optional[bool] = None
    if int(args.enable_thinking) in (0, 1):
        enable_thinking = bool(int(args.enable_thinking))

    prompt_cfg = PromptConfig(
        prompt_format="chat" if use_chat else "plain",
        system_prompt=(args.system_prompt or "").strip() if use_chat else "",
        add_generation_prompt=True,
        add_special_tokens_plain=bool(args.add_special_tokens_prompt),
        enable_thinking=enable_thinking if use_chat else None,
    )
    print(f"[Info] prompt_cfg={prompt_cfg}")
    ...
    if args.Qs_path:
        ...
        print("[Warn] If you compute Qs for Qwen, make sure your base script uses the SAME chat template formatting.")
    else:
    ...
    if n_flips_used == 0:
        out = {
            "meta": {
                ...
                "prompt_cfg": prompt_cfg.__dict__,
            },
            "scan_rows": scan_rows,
            "flip_rows": [],
        }
