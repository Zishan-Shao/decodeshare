...
OUTROOT="patch_back/results/${MODEL_QWEN}/layer${LAYER}"
mkdir -p "${OUTROOT}"

echo "[Info] MODEL_QWEN=${MODEL_QWEN}"
echo "[Info] OUTROOT=${OUTROOT}"
echo "[Info] LAYER=${LAYER} DTYPE=${DTYPE}"
...
CUDA_VISIBLE_DEVICES=3 python "${OPEN_SCRIPT}" \
  --base_script_path "${BASE_SCRIPT}" \
  --model "${MODEL_QWEN}" --device "${DEVICE}" --dtype "${DTYPE}" \
  --layer "${LAYER}" --seed "${SEED_MAIN}" \
...
# ============================================================
# (4) Summarize all qwen JSONs (MD/LaTeX + alpha sweep)
# ============================================================
SUM_DIR="${OUTROOT}/_summary"
mkdir -p "${SUM_DIR}"

echo "[Run] Summarize all JSONs under ${OUTROOT}"
CUDA_VISIBLE_DEVICES=3 python "${SUM_SCRIPT}" \
  --dir "${OUTROOT}" \
  --pattern "**/*.json" \
  --recursive \
  --no_dedupe \
...
# ============================================================
ANALYSIS_PY="${SUM_DIR}/analyze_qwen_results.py"
REPORT_MD="${SUM_DIR}/qwen_report.md"
TABLES_TEX="${SUM_DIR}/qwen_tables.tex"
...
echo "[Run] Analyze Qwen results -> report/md/tex/plots"
