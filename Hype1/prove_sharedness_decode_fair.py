...
HF_COMPAT_CACHE_ROOT = os.path.expanduser("~/.cache/huggingface/datasets_compat_rebuild")


def _is_dataset_cache_metadata_error(e: Exception) -> bool:
    return "must be called with a dataclass type or instance" in str(e)


def _compat_cache_dir(path: str, name: Optional[str]) -> str:
    parts = [path.replace("/", "__")]
    if name:
        parts.append(name.replace("/", "__"))
    return os.path.join(HF_COMPAT_CACHE_ROOT, "__".join(parts))
...
def _try_load_dataset(path: str, name: Optional[str] = None):
    try:
        ...
        if _is_dataset_cache_metadata_error(e):
            compat_cache_dir = _compat_cache_dir(path, name)
            print(
                f"[Warn] rebuilding stale HF cache: {path}"
                f"{'' if name is None else f'/{name}'}"
                f" -> cache_dir={compat_cache_dir}"
            )
            try:
                if name is None:
                    return load_dataset(path, cache_dir=compat_cache_dir, download_mode="force_redownload")
                return load_dataset(path, name, cache_dir=compat_cache_dir, download_mode="force_redownload")
            except Exception as e2:
                print(
                    f"[Warn] failed to rebuild dataset: {path}"
                    f"{'' if name is None else f'/{name}'} :: {repr(e2)}"
                )
...
def subspace_overlap_fro(Qref: np.ndarray, Q: np.ndarray) -> Dict[str, float]:
    # Both D×k, columns assumed orthonormal
    k = min(Qref.shape[1], Q.shape[1])
    A = Qref[:, :k]
    B = Q[:, :k]
    M = A.T @ B
    ...
    return {
        "k_overlap": int(k),
        "overlap_fro": float(np.mean(s ** 2)),               # = ||A^T B||_F^2 / k
        "min_cos": float(s.min()),
        "mean_cos": float(s.mean()),
        "max_angle_deg": float(np.degrees(np.arccos(s.min()))),
    }
...
def main():
    ...
    ap.add_argument("--tasks", type=str, default="all",
                help="Comma-separated task names to use, or 'all'. Example: gsm8k,boolq,piqa")
    ap.add_argument("--save_joint_basis", type=str, default="",
                    help="Optional .npy path to save pooled PCA basis Q (D×k).")
    ap.add_argument("--ref_joint_basis", type=str, default="",
                    help="Optional .npy path of reference basis Q_ref (D×k). If provided, compute overlap.")
    # Filter tasks if requested
    if args.tasks != "all":
        want = [x.strip() for x in args.tasks.split(",") if x.strip()]
        missing = [t for t in want if t not in prompts_by_task]
        if missing:
            raise RuntimeError(f"--tasks contains unknown tasks: {missing}. Available: {list(prompts_by_task.keys())}")
        prompts_by_task = {t: prompts_by_task[t] for t in want}
    ...
    try:
        ...
        # Optionally save joint basis
        if str(args.save_joint_basis).strip():
            os.makedirs(os.path.dirname(os.path.abspath(args.save_joint_basis)), exist_ok=True)
            np.save(args.save_joint_basis, Q)

        overlap_stats = None
        if str(args.ref_joint_basis).strip():
            Qref = np.load(args.ref_joint_basis).astype(np.float32)
            overlap_stats = subspace_overlap_fro(Qref, Q)
            print(f"[Overlap] ref={args.ref_joint_basis} overlap_fro={overlap_stats['overlap_fro']:.4f} "
                f"max_angle_deg={overlap_stats['max_angle_deg']:.2f} k={overlap_stats['k_overlap']}")
        ...
        out = {
            ...
            "observed": {
                ...
                "overlap_to_ref": (None if overlap_stats is None else overlap_stats),
            },
