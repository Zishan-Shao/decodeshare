# -*- coding: utf-8 -*-
"""
collect_decode_acts.py

一次性采集：每个 task 的 decode-phase (seq_len==1) last-token hidden states，
并做与 prove_sharedness_decode_fair.py 一致的公平预处理：
...
用法示例：
  CUDA_VISIBLE_DEVICES=0 python collect_decode_acts.py \
...
from prove_sharedness_decode_fair import (
    _should_write_txt,
    TeeStdout,
    set_global_seed,
    load_calib_prompts,
...
def main():
    ...
    try:
        ...
        # 1) load prompts（与 prove_sharedness_decode_fair.py 一致）
