...
Compute subspace convergence curves for 3 activation modes in one run:
  (a) decode-last     : decode phase seq_len==1 (like prove_sharedness_decode_fair.py)
...
如果你要跑你 full benchmark（你写的 prove_sharedness_decode_full.py 那套 loader），把 --loader full：
...
def load_prompts(loader: str, n_prompts: int, seed: int) -> Dict[str, List[str]]:
    ...
    loader:
      ...
      - full: tries to import prove_sharedness_decode_full and use its load_calib_prompts_full
    """
    ...
    if loader == "full":
        try:
            import prove_sharedness_decode_full as full  # your wrapper file
